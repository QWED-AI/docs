...
signature — every pair present in the directory is checked, one failure
never stops the rest, and the auditor works from the files alone:
...
# Verify every pair present in the directory the transfer arrived in
