...
signature — every pair in the transfer is checked, so no receipt can go
unaudited, and the auditor works from the files alone:
...
# Audit every exported pair in the directory the transfer arrived in
...
for json_path in pairs:
    base = json_path.stem  # "receipt-<receipt_id>"
    try:
        receipt_json = json_path.read_text(encoding="utf-8")
