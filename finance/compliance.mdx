...
computes it on demand from the full receipt. Export both artifacts so an
auditor can verify the receipt later:
...
# Store the signature beside the exported receipt
Path("receipt.json").write_text(receipt.to_json())
Path("receipt.sig").write_text(signature)
```
...
To verify an export later, parse `receipt.json`, recompute
`receipt.get_signature(signing_key)`, and compare with the stored
`receipt.sig` using `hmac.compare_digest` as shown in
[Cryptographic signature](#cryptographic-signature) above.
