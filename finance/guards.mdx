...
<Note>
**Since v2.2.0:** Black-Scholes uses `mpmath` (30 decimal places) for transcendental functions (`log`, `exp`, `sqrt`, `erf`) and `Decimal` for monetary outputs. Greeks are returned as Decimal-quantized **strings**, not `float`.
</Note>
...
# result.greeks = {"delta": "0.4502", "gamma": "0.038900", ...}
# Note: Greeks are returned as Decimal-quantized strings (since v2.2.0)
# to preserve precision and avoid IEEE-754 float artifacts.
```
...
<Note>
**Since v2.2.0:** All YTM, duration, and convexity calculations use `Decimal` arithmetic end-to-end (50-digit precision). The Newton-Raphson solver, accumulators, and `tolerance_pct` are all `Decimal`, eliminating IEEE-754 contamination.
</Note>
...
<Note>
**Since v2.2.0:** All metrics — VaR, beta, Sharpe, Sortino, max drawdown, information ratio — use `Decimal` arithmetic with `Decimal.sqrt()`. The `Z_SCORES` lookup table uses `Decimal` constants. Beta accumulation no longer suffers from catastrophic cancellation.
</Note>
