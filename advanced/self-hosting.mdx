...
Scraping `/metrics` requires a platform operator: add the scraper identity's user ID to `QWED_METRICS_OPERATOR_USER_IDS` (fail-closed when unset — every caller is denied, including org owners/admins). See [All-tenant metrics authorization](/advanced/security-hardening#all-tenant-metrics-authorization).
