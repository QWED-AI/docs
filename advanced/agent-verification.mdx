...
QWED fails closed when an `action_type` does not have explicit registered semantics. An action type is recognized when it appears in either the action-engine map (e.g. `execute_sql`, `execute_code`, `verify_logic`) or the tool risk map (e.g. `calculate`, `read_file`, `database_read`).

If an unregistered action is submitted, risk assessment and verification checks are skipped entirely and the call is denied with `QWED-AGENT-ACTION-001`:
...
The denial response intentionally omits the `verification` block because no engine is bound to the action. The in-flight step reservation is released, so the agent can retry the same `step_number` with a registered action.

The `verification.engine` field in approved responses now always reflects the explicitly registered engine. The previous generic `"security"` fallback is no longer emitted.
