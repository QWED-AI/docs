...
Every `verify_action` call must reference an `action_type` that QWED knows how to verify. Action types are either bound to a verification engine (for example `verify_math` → `math`, `execute_sql` → `sql`) or registered as a governed tool with a known risk level (for example `read_database`, `send_email`). Action types outside those two registries are denied with `QWED-AGENT-ACTION-001` before any risk assessment runs.
