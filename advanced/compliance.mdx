...
3.  **Persisted chain continuity:**
    On each append, `AuditLogger` looks up the latest persisted entry hash for the writing organization and binds the new entry to it. Each organization has its own chain head, tracked independently. If the in-memory chain head diverges from the persisted head at write time, the logger raises `SecurityError` and rejects the write rather than starting a fresh chain.

    Before appending, the logger re-verifies the persisted chain head and refuses to extend a chain whose latest entry fails integrity checks. On databases that support it, the chain-head read is taken with a `SELECT ... FOR UPDATE`; on SQLite, the append session opens an `IMMEDIATE` transaction so chain-head validation and the append remain serialized.
