...
Cryptographic logging is enabled by default, but you **must** configure the signing key before the `AuditLogger` can initialize. The logger fails closed if the key is missing.

1.  **Set Environment Variable:**
    Set `QWED_AUDIT_SECRET_KEY` to a secure, random string (at least 32 characters).
    ```bash
    export QWED_AUDIT_SECRET_KEY="your-secure-production-key-here"
    ```
    *Warning: If `QWED_AUDIT_SECRET_KEY` is unset, `AuditLogger()` raises `SecurityError` at construction time and no audit entries can be written.*
