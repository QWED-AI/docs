...
In a Docker Compose environment, the `qwed-core` container mounts the host's Docker socket (`/var/run/docker.sock`). Ensure the user running Docker Compose has appropriate permissions.
