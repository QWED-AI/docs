...
| The identity endpoint is unreachable | Restart the local server, then re-run `qwed init` |
...
| The server returns `503` — it is running with no secrets configured | Set `QWED_JWT_SECRET_KEY` and `QWED_API_KEY_LOOKUP_SECRET` in the server's environment, then re-run `qwed init` |
