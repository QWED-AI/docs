...
1. `qwed init` generates a fresh random nonce for each run.
...
The secrets themselves never leave the server. Because each proof is bound to a one-time nonce, a captured response cannot be replayed in a later handshake.
