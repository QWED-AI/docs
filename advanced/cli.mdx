...
3. **API key bootstrap** — starts a local QWED server, verifies the server's identity, creates an organization, and generates a one-time API key you can use immediately
...
The key above is shown in QWED's documented [masked form](/api/authentication#masking-keys) — a real `qwed init` run prints the full v2 key (`qwed_live_` + 36 characters) once, at that moment.
...
<Note>
API keys are never logged or displayed in full. The `.env` file is written atomically with owner-only permissions (`0600` on Unix) and symlink protection. `qwed init` auto-generates two server secrets on first run. `QWED_JWT_SECRET_KEY` signs JWTs. `QWED_API_KEY_LOOKUP_SECRET` derives the HMAC-SHA256 digests the server uses to look up API keys. Both are passed into the local server it starts. The fail-closed import check at server boot no longer trips on a fresh install.
</Note>
...
Before bootstrapping an API key, `qwed init` proves that the server answering on `--server-url` holds this project's secrets. A healthy `GET /health` only shows that something answers on the port. A foreign process, or a stale QWED server started with older secret values, would otherwise receive the bootstrap request and issue a key that the intended server cannot resolve after a restart.

The handshake runs on both paths: when `qwed init` reuses an already-running server, and when it spawns a fresh one.

1. `qwed init` generates a fresh random nonce for each run (24 random bytes, URL-safe Base64).
2. It calls [`GET /health/identity`](/api/endpoints#get-health-identity) with that nonce.
3. The server returns HMAC-SHA256 proofs over the nonce, one under `QWED_JWT_SECRET_KEY` and one under `QWED_API_KEY_LOOKUP_SECRET`.
4. `qwed init` computes the expected proofs from its local secrets and compares them in constant time.

The secrets themselves never leave the server. Because each proof is bound to a one-time nonce, a captured response cannot be replayed in a later handshake. A narrow residual race is accepted by design: a local process that binds the port after it is released between the proof and the bootstrap could receive the key without holding either secret. Defending against that active local attacker is out of scope — the handshake addresses accidental staleness — so bootstrap only on hosts you control.

The handshake fails closed. `qwed init` prints `[x] Server identity check failed: <reason>`, exits with status `1`, and does not bootstrap a key when:

| Condition | Remediation |
|---|---|
| Local `QWED_JWT_SECRET_KEY` or `QWED_API_KEY_LOOKUP_SECRET` is empty | Re-run `qwed init` so fresh secrets are generated and persisted |
| The identity endpoint is unreachable | Start (or restart) the local server, then re-run `qwed init` |
| The server returns `404` — an older server without the endpoint | Upgrade the server to a version that exposes `GET /health/identity`, then re-run `qwed init` |
| The server returns `503` — it is running with no secrets configured | Set `QWED_JWT_SECRET_KEY` and `QWED_API_KEY_LOOKUP_SECRET` in the server's environment, restart the server so it loads them, then re-run `qwed init` |
| The server returns any other non-`200` status | Restart the local server, then re-run `qwed init` |
| The response body is malformed | Restart the local server, then re-run `qwed init` |
| One or both proofs don't match | Restart the server so it loads this project's `.env` secrets, then re-run `qwed init` |

<Warning>
Never bootstrap API keys against a server you don't recognize. If the identity check fails, stop whatever process holds the port before you re-run `qwed init`.
</Warning>
...
**Re-running `qwed init`:** Running the command again merges new values into your existing `.env` file. Existing variables that you don't reconfigure are preserved. Both server secrets — `QWED_JWT_SECRET_KEY` and `QWED_API_KEY_LOOKUP_SECRET` — are retained across re-runs and never rotated automatically, because rotating the lookup secret invalidates every previously issued API key's lookup digest. If your `.env` already sets `QWED_API_KEY_LOOKUP_SECRET` equal to `QWED_JWT_SECRET_KEY`, `qwed init` refuses to continue: reusing the JWT secret is a value the server will not boot with. Remove the offending line (a fresh, distinct value is generated) or set a distinct value manually. See [API key storage and migration](/api/authentication#api-key-storage-and-migration-v7-2) for why the lookup secret must stay stable.
...
| `QWED_JWT_SECRET_KEY` | All | JWT secret for local server authentication (auto-generated by `qwed init`, retained on re-run) |
| `QWED_API_KEY_LOOKUP_SECRET` | All | HMAC-SHA256 key used to derive API-key lookup digests (auto-generated by `qwed init`, retained on re-run, must differ from `QWED_JWT_SECRET_KEY`). Never rotated automatically — see [API key storage and migration](/api/authentication#api-key-storage-and-migration-v7-2) |
