...
Interactive wizard that configures your LLM provider and writes credentials to a `.env` file with restrictive permissions (0600 on Unix).
...
1. **Provider selection** — choose from OpenAI, Anthropic, Ollama, or any OpenAI-compatible endpoint (Groq, Together, LM Studio, etc.)
2. **Credential entry** — API keys are entered with hidden input and validated against provider-specific patterns
3. **Connection test** — optional lightweight connectivity check against the provider's read-only `/models` endpoint (5 s timeout)
4. **Default provider** — set the selected provider as `ACTIVE_PROVIDER` so `qwed verify` auto-detects it
5. **`.gitignore` protection** — verifies `.env` is in `.gitignore`; offers to add it if missing, and aborts if you decline
...
Select your LLM provider:
  ...
  4. OpenAI-Compatible (DO, Groq, Together, LM Studio, etc.)
