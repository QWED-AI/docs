...
<Note>
API keys are never logged or displayed in full. The `.env` file is written atomically with owner-only permissions (`0600` on Unix) and symlink protection. `qwed init` also auto-generates two server secrets on first run — `QWED_JWT_SECRET_KEY` (used to sign JWTs) and `QWED_API_KEY_LOOKUP_SECRET` (used to derive the HMAC-SHA256 digests the server uses to look up API keys). Both are threaded into the local server it starts, so the fail-closed import check at server boot no longer trips on a fresh install.
</Note>
