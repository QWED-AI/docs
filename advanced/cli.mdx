...
### `qwed init` - Secure provider setup

Interactive wizard that configures your LLM provider and writes credentials to a `.env` file with restrictive permissions. Run this once after installing QWED.
...
2. **Credential entry** — API keys are masked during input and never displayed on screen
3. **Key format validation** — checks that your key matches the provider's expected format before making any network call
4. **Connection test** (optional) — verifies the key works by hitting a read-only endpoint
5. **Default provider** — sets the chosen provider as the active default so `qwed verify` works immediately
6. **`.gitignore` protection** — confirms `.env` is gitignored before writing secrets, and offers to add it if missing
...
Set OpenAI as default active provider? [Y/n]: y

Verified: .gitignore includes .env

Written to /your/project/.env
...
| Provider | Auth | Key format |
|----------|------|------------|
| OpenAI | API key | `sk-...` or `sk-proj-...` |
| Anthropic | API key | `sk-ant-...` |
| Ollama (local) | None | No key needed |
| OpenAI-compatible | Endpoint + key | Any bearer token |

The OpenAI-compatible option works with DigitalOcean, Groq, Together AI, LM Studio, and any service that exposes an OpenAI-compatible API.
...
2. **`ACTIVE_PROVIDER` env var** (set by `qwed init`)
...
- [QWEDLocal guide](/advanced/qwed-local) — Python API
