...
The secrets themselves never leave the server. Because each proof is bound to a one-time nonce, a captured response cannot be replayed in a later handshake. A narrow residual race is accepted by design: a local process that rebinds the port in the moment between the proof and the bootstrap can bypass the check, but an attacker able to do that already reads this project's secrets at rest, which is outside the accidental-staleness threat model this handshake addresses.
...
| The server returns a non-`200` status, such as `404` from an older server without the endpoint or `503` from a server with no secrets configured | Restart the local server, then re-run `qwed init` |
