...
<Info>New in v4.0.4 · Updated in v5.0.1</Info>
...
The legacy `CodeExecutor` class, which previously executed arbitrary Python through raw `exec()`, has been permanently disabled. Any attempt to call `CodeExecutor.execute()` raises a `RuntimeError`:
...
This hard-block applies to all callers, regardless of input. The class retains its interface for backward compatibility but enforces a fail-closed contract — no code path can reactivate in-process execution. All code execution must go through `SecureCodeExecutor`, which provides Docker container isolation, resource limits, network isolation, and pre-execution AST validation.
