...
Handling is idempotent. If the same leak is redelivered, or two deliveries race, exactly one revokes the key and sends the email. The other is treated as already revoked. Test keys with the `qwed_test_` prefix are excluded from scanning, so they never trigger revocation.
