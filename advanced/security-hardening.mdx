...
#### Fail-closed when `CodeVerifier` cannot be imported

<Info>Updated May 2, 2026</Info>

`SecureCodeExecutor` delegates code-safety verification to `qwed_new.core.code_verifier.CodeVerifier`. If that import fails — for example, after a partial install or a version mismatch — the executor now blocks execution instead of falling back to a permissive keyword scan.

When `CodeVerifier` is unavailable, `SecureCodeExecutor.execute()` returns:
...
The container is never started in either case. To restore execution, fix the underlying import error so `from qwed_new.core.code_verifier import CodeVerifier` succeeds. Do not rely on the heuristic keyword scan as a substitute for the structured verifier.
