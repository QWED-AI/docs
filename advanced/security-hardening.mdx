...
<Warning>
Fail-closed by design: with `QWED_METRICS_OPERATOR_USER_IDS` unset, `/metrics` and `/metrics/prometheus` deny **every** caller — including organization owners and admins. Self-service signup mints `role="owner"` for every new account, so org roles cannot be trusted as platform-wide authority. If your monitoring stack scrapes `/metrics`, create a dedicated operator account, mint it an API key, and add its user ID to the allowlist **before deploying**.
</Warning>
