...
If your monitoring stack scrapes `/metrics/prometheus`, create a dedicated operator account, mint it an API key, and add its user ID to the allowlist **before deploying**. Prometheus cannot template environment variables into its own config, so inject the key into the scrape config through your secret-management pipeline.
</Warning>
