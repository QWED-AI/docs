...
> Closes issue #166 by ensuring `SecureCodeExecutor` blocks execution when the primary `CodeVerifier` import fails, rather than degrading to a heuristic keyword scan as the authorization gate.

### What changed

- **Removed permissive fallback** — `SecureCodeExecutor._is_safe_code()` no longer returns the result of `_basic_safety_check()` as the authorization decision when `CodeVerifier` import fails.
...
- **Regression coverage** — `tests/test_secure_executor_coverage.py` now asserts that import failure blocks otherwise safe code, that `containers.run()` is never invoked when the verifier is unavailable, and that the advisory fallback only adds context to the error message.

### Why

If the primary verification boundary is unavailable, execution must not proceed. A heuristic blacklist can be useful for diagnostics, but it must not replace the main verifier as an execution gate.

See the [security hardening guide](/advanced/security-hardening#codeverifier-import-failure-blocks-execution) for the updated behavior.
