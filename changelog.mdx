...
## QWED-Verification — `VerificationCache` keys bound to trust context
**Released: May 17, 2026**

> Hardens `qwed_sdk.cache.VerificationCache` so a cache hit requires an exact match of both the normalized query **and** all trust-bound context dimensions (provider, model, policy version, tenant/session, environment fingerprint). Closes Issue #187.

### What changed

- **New `CacheContext` dataclass** — Carries `provider`, `model`, `policy_version`, and optional `tenant_id` and `env_fingerprint`. All fields participate in the cache key.
- **Composite, context-bound key** — Key is `SHA-256(normalized_query)` × `SHA-256(canonical_context_json)`. Two identical query strings under different contexts are stored as independent entries and never collide.
- **Schema v2 (`cache_v2`)** — New table with composite `PRIMARY KEY (key, context_fingerprint)` and a per-entry `context_fingerprint` column. The fingerprint is re-validated on every read as defence-in-depth.
- **Legacy entries never returned** — Rows in the v1 `cache` table (no context fingerprint) are unreachable from any code path.
- **Context-mismatch is a deterministic miss** — Provider switch, model change, policy version change, tenant/session change, and env fingerprint change all yield `None`.

### Breaking changes

- `VerificationCache.get()` and `VerificationCache.set()` now require a `CacheContext` argument. Calls like `cache.get("2+2")` or `cache.set("2+2", result)` raise `TypeError`.
...
# Provider switch — replay prevented
ctx2 = CacheContext(provider="claude", model="claude-opus-4-5", policy_version="v1")
cache.get("2+2", ctx2)             # None
```

See [Python SDK — Verification cache](/sdks/python#verification-cache) for the full API reference.
