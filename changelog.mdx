...
- **CryptoTaxGuard** — Zero VDA income now verifies a claimed tax of zero, instead of returning `verified=True` regardless of claim. Negative VDA income (a loss) is treated as a loss and routed to set-off logic, not silently treated as "no income."
