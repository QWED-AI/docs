---
title: "Changelog"
description: "Release notes for QWED Protocol: version history, new guards, breaking changes, security fixes, and audit-driven hardening across all QWED engines."
---
...
## QWED-Tax — fail-closed on ambiguous classification and claim comparison

**Released: June 21, 2026**

> Six tax guards no longer return success when the inputs are ambiguous or when the agent's claim was never compared against the computed truth. Where a guard previously guessed a classification or returned a calculation as if it were a verification, it now blocks or surfaces an explicit `computed_only` flag.

### What changed

- **ClassificationGuard (IRS Common Law)** — `verify_worker_status` returns `None` (and `verify_classification_claim` returns `verified=False` with `"Ambiguous classification"`) when behavioral, financial, and relationship indicators are mixed. The previous default-to-contractor path on partial employee indicators has been removed.
- **CapitalGainsGuard** — `determine_term` raises `ValueError` on unparseable dates and unknown asset types instead of returning a sentinel string or fabricating a 1095-day threshold. `TaxPreFlight._check_capital_gains` catches the error and produces a structured block. `verify_tax_rate` now returns `verified=False` for `SLAB`-rated assets (e.g. `debt_LTCG`) because slab rates depend on the filer's bracket.
- **SpeculationGuard** — `verify_setoff` requires loss/profit sources to match a known vocabulary (`intraday`, `f&o`, `f_o`, `futures`, `options`, `delivery`, `business`, `capital_gains`). Unrecognized strings return `verified=False` instead of being treated as non-speculative by substring match.
- **InterHeadAdjustmentGuard** — explicit allowlist (`HOUSE_PROPERTY`, `BUSINESS_NON_SPECULATIVE`, `OTHER_SOURCES`) plus prohibition matrix. `SALARY` losses are now blocked against every other head, and heads outside both lists return `verified=False` with a manual-review message. The previous "default allow" path is gone.
- **GSTGuard RCM** — `verify_rcm_applicability` accepts an optional `claimed_is_rcm` boolean. When provided, the guard compares the computed RCM liability against the claim and returns `verified=True` only on exact match. When omitted, the response carries `computed_only=True` so callers can distinguish a calculation from a verification. Unknown service or entity values fail closed instead of being coerced to `OTHER` / `INDIVIDUAL`.
- **CryptoTaxGuard** — `verify_flat_tax_rate` now distinguishes zero income (verifies `claimed_tax == 0`) from a loss (`vda_income < 0` returns `verified=False` and routes to `verify_set_off`). The previous behavior of treating any non-positive income as "no income" and ignoring `claimed_tax` has been removed.

### What this means for you

If your agent relied on `verified=True` for mixed classification facts, unparseable dates, unknown trading sources, or unrecognized GST services, those calls will start blocking. If you call `verify_rcm_applicability` without `claimed_is_rcm`, expect a `computed_only=True` flag in the response — pass the agent's claim to get a real verification result. Crypto agents that reported a loss as `vda_income=-X` must now route through `verify_set_off`.
...
**Released: June 19, 2026** · [GitHub PR #41](https://github.com/QWED-AI/qwed-tax/pull/41)

> Closes a fail-open gap across six QWED-Tax guards. Inputs that hit an unmodeled rule path (unknown service type, state, asset class, payment type, address jurisdiction, or ITC category) no longer return a quiet `verified=True`. They now return `verified=False` with a structured error, so an upstream LLM cannot route a payment through a category the guard has not been configured to evaluate.

### What changed

- **`TDSGuard.calculate_deduction`** — Unknown `service_type` returns `{"verified": False, "error": "No TDS rule configured for service type '<TYPE>'. Cannot verify — block pending rule configuration."}`. Previously returned `{"verified": True, "deduction": "0"}`, which `TaxPreFlight._check_invoice_tds` accepted as a clean payment with zero withholding.
- **`CapitalGainsGuard.verify_tax_rate`** — Unknown `(asset_type, term)` pair returns `{"verified": False, "error": "No statutory rate configured for <asset>_<term>. Cannot verify claimed rate."}`. The "no hard constraint, assume verified" branch has been removed.
- **`NexusGuard.check_nexus_liability`** — Unconfigured `state` returns `{"verified": False, "error": "State <CODE> not in configured nexus threshold table. Cannot verify nexus liability — block pending rule configuration."}` instead of `verified=True` "not in high-risk list".
- **`AddressGuard.verify_address`** — Unmodeled state returns `{"verified": False, "message": "State <CODE> not in validation database. Address cannot be auto-verified — manual review required."}` instead of `verified=True` "assumed valid".
- **`Form1099Guard.verify_filing_requirement`** — Unmodeled `payment_type` returns `{"filing_required": "UNVERIFIABLE", "form": None, "reason": "No filing rule configured for payment type '<TYPE>'. Cannot verify filing requirement — manual determination required."}`. Previously returned `filing_required=False`.
- **`InputCreditGuard`** — ITC under GST is "allowed unless specifically blocked", so the default verdict for unknown categories is still `verified=True`. The response now also carries `"unverified_category": True` and `audit_trace.inputs.category_match: "default_allow"` so consumers can distinguish a known-eligible category from a default-allow fallback.
...
| Guard | Trigger | Before | After |
| --- | --- | --- | --- |
| `TDSGuard` | Unknown `service_type` | `verified=True`, `deduction="0"` | `verified=False` with error |
| `CapitalGainsGuard` | Unknown `(asset_type, term)` | `verified=True`, "no hard constraint" | `verified=False` with error |
| `NexusGuard` | Unknown `state` | `verified=True`, "not in high-risk list" | `verified=False` with error |
| `AddressGuard` | Unknown `state` | `verified=True`, "assumed valid" | `verified=False`, "manual review required" |
| `Form1099Guard` | Unknown `payment_type` | `filing_required=False` | `filing_required="UNVERIFIABLE"`, "manual determination required" |
| `InputCreditGuard` | Unknown category | `verified=True` (silent) | `verified=True` + `unverified_category=True` |

### Breaking changes

<Warning>
  Any integration that branched only on `verified=True` from `TDSGuard`, `CapitalGainsGuard`, `NexusGuard`, or `AddressGuard` to mean "proceed" must now also handle a `verified=False` response with `error` for the unknown-rule path. The recommended action is to surface the error to a human reviewer rather than retry with a different input.
</Warning>

<Warning>
  `Form1099Guard.verify_filing_requirement` may now return `filing_required="UNVERIFIABLE"` (a string), not just `True` or `False`. Callers that did `if result["filing_required"]:` will incorrectly treat `"UNVERIFIABLE"` as truthy. Update consumers to branch on the three-state value explicitly.
</Warning>
...
result = Form1099Guard().verify_filing_requirement(payment)

# Before
if result["filing_required"]:
    file_form(result["form"])

# After
filing = result["filing_required"]
if filing is True:
    file_form(result["form"])
elif filing == "UNVERIFIABLE":
    escalate_to_reviewer(payment, result["reason"])
# filing is False -> below threshold, nothing to file
```

See the [QWED-Tax guards reference](/tax/guards) for the updated fail-closed notes on each guard.
