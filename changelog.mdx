...
- **Composite, context-bound key** — Key is `SHA-256(normalized_query)` × `SHA-256(canonical_context_json)`. Two identical query strings under different contexts occupy independent entries and never collide.
- **Schema v2 (`cache_v2`)** — New table with composite `PRIMARY KEY (key, context_fingerprint)` and a per-entry `context_fingerprint` column. `get()` re-validates the fingerprint on every read as defence-in-depth.
- **Legacy entries never returned** — No code path reaches rows in the v1 `cache` table (which lack a context fingerprint).
