...
- **SafetyGuard recursive extraction.** SafetyGuard scans string content at any nesting depth (bounded to 12 levels), including the canonical `choices[].message.content` shape, so PII, injection, and harmful patterns inside nested structures are detected.
