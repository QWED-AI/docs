---
title: "QWED changelog — releases, breaking changes, and fixes"
...
- A `VERIFIED` math result is only surfaced once `create_verification_attestation()` returns `ISSUED`. Any attestation signing failure downgrades the response to `UNVERIFIABLE` — the error code is echoed under `trust_boundary.attestation_error` and no token is returned.
...
- If your integration branched on `trust_boundary.overall_status`, it now reflects the post-attestation decision. Signing outages surface as `UNVERIFIABLE` with `attestation_error`, not as a raw `VERIFIED` verdict.
