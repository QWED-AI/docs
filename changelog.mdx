...
## QWED-Finance — Float-to-Decimal/mpmath guard migration
**Released: May 2, 2026** · [GitHub PR #23](https://github.com/QWED-AI/qwed-finance/pull/23)

> Eliminates IEEE-754 float contamination from `BondGuard`, `DerivativesGuard`, and `RiskGuard`. All financial math now uses `Decimal` (exact arithmetic) or `mpmath` (arbitrary-precision transcendentals), aligning with the precision standards already used by `TradingGuard` and `FinanceVerifier`.

### What changed

- **`BondGuard` (C-01)** — `_solve_ytm()` Newton-Raphson, `verify_ytm()`, `verify_duration()`, and `verify_convexity()` now run end-to-end on `Decimal`. `tolerance_pct` is stored as `Decimal`.
- **`DerivativesGuard` (C-02)** — `verify_black_scholes()`, `_norm_cdf()`, `_norm_pdf()`, `verify_put_call_parity()`, and `verify_delta()` use `mpmath.log/exp/sqrt/erf` at 30 decimal places. `verify_margin_call()` uses `Decimal` comparison.
- **`RiskGuard` (C-03)** — `verify_var()`, `verify_beta()`, `verify_sharpe_ratio()`, `verify_sortino_ratio()`, `verify_max_drawdown()`, and `verify_information_ratio()` use `Decimal` accumulation and `Decimal.sqrt()`. `Z_SCORES` lookup constants are `Decimal`.
- **Greeks output type** — `_calculate_greeks()` returns `Decimal.quantize()` strings instead of rounded floats.
- **Catastrophic cancellation fix** — Beta accumulation in `RiskGuard` no longer suffers from float cancellation errors on small return series.
- **`BondGuard._parse_rate()` return type** — Now returns `Decimal` instead of `float`, removing the `Decimal(str(float(...)))` round-trip at every call site.
- **Money formatting** — Bond accrued-interest and dirty-price outputs use `Decimal.quantize()` instead of `f"{x:.2f}"`, eliminating the last `float`-style format specifiers.

### Breaking changes

<Warning>
Greeks returned by `DerivativesGuard.verify_black_scholes()` are now Decimal-quantized **strings**, not floats.
...
Consumers that performed arithmetic on Greeks must cast via `Decimal(greeks["delta"])` before computing.
</Warning>

<Note>
`mpmath` is now a runtime dependency of `qwed-finance`. It is already available as a transitive dependency via `sympy`, so most installations require no change.
</Note>
...
| --- | --- | --- |
| C-01 | `BondGuard` float contamination | `Decimal` end-to-end |
| C-02 | `DerivativesGuard` `math.*` float leak | `mpmath` (30 dp) + `Decimal` outputs |
| C-03 | `RiskGuard` `math.sqrt` and float accumulation | `Decimal.sqrt()` + `Decimal` accumulation |

### Test coverage

23 new tests added in `tests/test_float_contamination.py`:
