...
<Warning>
  Callers that previously relied on the 0.1-rupee tolerance in `CryptoTaxGuard.verify_flat_tax_rate` will now receive `verified=False` for claims that differ from `vda_income * 0.30` by more than 1 paise. Round your claimed tax to two decimal places with `ROUND_HALF_UP` before calling the guard.
