...
Jump to: [`qwed init` server identity handshake](#qwed-verification-—-qwed-init-server-identity-handshake) · [Leaked key auto-revocation](#qwed-verification-—-leaked-api-key-auto-revocation-and-false-positive-feedback) · [API-key format spec](#qwed-verification-—-canonical-api-key-format-spec) · [Signed secret-scanning webhook](#qwed-verification-—-signed-secret-scanning-webhook-endpoint) · [v7.2.1](#v7-2-1-—-math-answer-comparison-security-patch) · [v7.2.0](#v7-2-0-—-precision-advisory-and-security-hardening-batch) · [v7.1.0](#v7-1-0-—-verification-context-v1-0-rollout) · [v7.0.0](#v7-0-0-—-full-diagnosticresult-engine-conformance)

---

## QWED Verification — `qwed init` server identity handshake

**Released: September 23, 2026** · [GitHub PR #388](https://github.com/QWED-AI/qwed-verification/pull/388) · Closes [issue #376](https://github.com/QWED-AI/qwed-verification/issues/376)

> `qwed init` now proves that the local server holds this project's secrets before it bootstraps an API key. Previously, a healthy `/health` only proved that something answered on the port. A foreign process or a stale server running older secret values could receive the bootstrap and issue a key that stops working after a restart.

### What changed

- **New `GET /health/identity` endpoint.** Takes a caller-supplied `nonce` (1–128 characters) and returns HMAC-SHA256 proofs over it under `QWED_JWT_SECRET_KEY` and `QWED_API_KEY_LOOKUP_SECRET`. Returns `400` for a missing or oversized nonce and `503` when server secrets aren't configured. The secrets never leave the server. See [`GET /health/identity`](/api/endpoints#get-health-identity).
- **Nonce-bound, replay-safe handshake.** `qwed init` generates a fresh nonce on every run and compares the proofs against locally computed values in constant time. A captured response is useless for any later handshake.
- **Runs on reused and freshly spawned servers.** A reused server can be foreign or stale even when secrets were retained. A fresh spawn can lose a port race after the health poll.
- **Fails closed.** A mismatch, a `404` from an older server, any other non-`200` status, a malformed response, an unreachable server, or missing local secrets stops `qwed init` with exit status `1` before any key is issued. Each message tells you to restart the local server or re-run `qwed init`.

See [`qwed init`](/advanced/cli#qwed-init-onboarding-and-bootstrap) and [troubleshooting](/troubleshooting#qwed-init-fails-with-server-identity-check-failed).

---

## QWED Verification — leaked API key auto-revocation and false-positive feedback

**Released: September 22, 2026** · [GitHub PR #380](https://github.com/QWED-AI/qwed-verification/pull/380) · [GitHub PR #386](https://github.com/QWED-AI/qwed-verification/pull/386) · Closes [issue #368](https://github.com/QWED-AI/qwed-verification/issues/368) and [issue #369](https://github.com/QWED-AI/qwed-verification/issues/369)

> Verified reports from `POST /webhooks/secret-scanning` now trigger enforcement. QWED automatically revokes any matching active API key, records a security event, and emails the key owner. An opt-in reporter can send true-positive and false-positive labels back to the scanning service.

### What changed

- **Automatic revocation.** A verified match against an active key sets `is_active=False` and `revoked_at`, and writes a `SECRET_LEAK_AUTO_REVOKED` security event. The event records only the `key_preview`. The plaintext token is never persisted, logged, or emailed.
- **Owner notification.** QWED emails the key's user, falling back to the organization's first active owner or admin. SMTP phases time out after 10 seconds. Revocation is committed first and is never rolled back if notification fails.
- **Idempotent handling.** Already-revoked keys and unknown tokens are not acted on. Redelivered or concurrent reports revoke and notify exactly once. Registered scanner patterns cover `qwed_live_` keys only, so `qwed_test_` keys are never reported.
- **False-positive feedback reporter (opt-in).** Set `QWED_LEAK_FEEDBACK_ENABLED=true` and `QWED_LEAK_FEEDBACK_URL` to POST one label per match after the revocation attempt. Items carry SHA-256 token hashes by default. `QWED_LEAK_FEEDBACK_SEND_RAW=true` is a separate explicit opt-in to send plaintext tokens, and only works with an HTTPS URL.

See [Security hardening — Automatic revocation of leaked keys](/advanced/security-hardening#automatic-revocation-of-leaked-keys) for the full behavior and configuration.

---

## QWED Verification — canonical API-key format spec

**Released: September 22, 2026** · [GitHub PR #387](https://github.com/QWED-AI/qwed-verification/pull/387) · Closes [issue #370](https://github.com/QWED-AI/qwed-verification/issues/370)

> QWED now publishes a canonical, machine-readable specification of its API-key formats, with lookaround-anchored scanning regexes and synthetic test vectors. Use it to build secret scanners, leak detectors, and SDK-side format validators.

### What changed

- **v2 checksummed format documented.** Current keys match `qwed_live_[A-Za-z0-9]{36}`: 30 random Base62 characters followed by a 6-character Base62 CRC32 checksum that you can validate offline.
- **Legacy v1 still accepted.** Keys matching `qwed_live_[A-Za-z0-9_-]{43}` keep working and are never force-rotated.
- **`qwed_test_` exclusion.** Scanners must exclude test keys from high-confidence production-secret matching.
- **Lookaround-anchored regexes.** Published patterns use charset-complement lookarounds instead of `\b`, because a v1 body can end in `-`. RE2-compatible delimiter-class alternatives are included.
- **Masking guidance.** Masked previews show the prefix, asterisks, and only the last 4 characters.
- **Machine-readable spec and test vectors.** `spec/api-key-format.json` and `spec/api-key-test-vectors.json` in the `qwed-verification` repository.

See [API key format](/api/authentication#api-key-format) for patterns, checksum details, and scanning rules.

---

## QWED Verification — signed secret-scanning webhook endpoint

**Released: September 20, 2026** · [GitHub PR #374](https://github.com/QWED-AI/qwed-verification/pull/374) · Closes [issue #367](https://github.com/QWED-AI/qwed-verification/issues/367)

> New `POST /webhooks/secret-scanning` endpoint receives signed reports of leaked QWED API keys from GitHub secret scanning and compatible partner scanners. The endpoint is unauthenticated at the transport layer by design — external scanners have no QWED credentials — and the ECDSA-NIST-P256V1-SHA256 signature over the raw request body is the only credential. Verification runs before any JSON parsing, logging, or database work, and every failure mode fails closed to a distinct status code.

### What changed

- **New public endpoint.** `POST /webhooks/secret-scanning` accepts a JSON array of leaked-token matches, verifies the `Github-Public-Key-Identifier` and `Github-Public-Key-Signature` headers against GitHub's published signing keys, and hands verified batches to the revocation sink in the background. See [`POST /webhooks/secret-scanning`](/api/endpoints#post-webhooks-secret-scanning) for the full request/response contract.
- **Raw-body signature verification.** The signature is verified over the exact bytes received; the body is never re-serialized. Nothing in the payload is parsed or logged until the signature is proven genuine.
- **P-256 pinning at key-load.** Signing keys are pinned to `secp256r1` when the PEM is loaded, so a future keys-list entry on another curve is dropped at load rather than silently trusted at verify time.
- **6-hour key cache with rotation retry.** Signing keys are cached in-process for 6 hours with an `ETag`-conditional refetch. On an unknown `key_identifier`, QWED refetches the list exactly once (throttled to one forced refetch per 60 s after a success, 10 s after a failure) and retries. A dead or throttled key service fails closed to `503`.
- **Timing-safe verdict comparison.** Success and failure branches run through `hmac.compare_digest` on a fixed-shape verdict, and every failure returns the same generic detail so the failing step is not observable.
- **Fail-closed status separation.** `401` for missing headers, `403` for a proven-bad signature against a fresh anchor, `503` for any state where verification was impossible (signing-key service down, anchor unusable, rotation retry throttled). `413` for over-cap bodies (1 MiB) or batches (500 matches); `400` for a malformed envelope. Tokens never appear in logs.
- **Optional keys-fetch token.** `GITHUB_KEYS_TOKEN` (or `GITHUB_TOKEN` as a fallback) can be set to raise the outbound rate limit on the keys request. Neither token grants any authority over the incoming webhook — only the signature does.

See [Security hardening — Secret-scanning webhook intake](/advanced/security-hardening#6-secret-scanning-webhook-intake) for the full trust model and operator configuration.

---

## QWED Verification — `qwed init` provisions `QWED_API_KEY_LOOKUP_SECRET`

**Released: September 21, 2026** · [GitHub PR #375](https://github.com/QWED-AI/qwed-verification/pull/375) · Closes [issue #372](https://github.com/QWED-AI/qwed-verification/issues/372)

> Fresh `qwed init` runs no longer crash the child server at boot on the fail-closed `QWED_API_KEY_LOOKUP_SECRET` import check. The CLI now generates the lookup secret alongside `QWED_JWT_SECRET_KEY`, writes it to `.env`, and threads it into the local server it starts.

### What changed

- **Auto-provisioned on first run.** `qwed init` generates a URL-safe random `QWED_API_KEY_LOOKUP_SECRET` (at least 48 bytes) whenever one is not already set, persists it to `.env` with the same `0600` permissions as other credentials, and exports it into the child server's environment.
- **Retained on re-runs, never rotated.** If `.env`, the process environment, or the project-root `.env` already holds a value, `qwed init` reuses it verbatim. Rotating this secret invalidates every stored API-key lookup digest, so the CLI will not do it for you.
- **Refuses to persist an unbootable value.** If the existing `QWED_API_KEY_LOOKUP_SECRET` equals `QWED_JWT_SECRET_KEY`, `qwed init` exits with an error instead of writing a value the server would refuse to start with. Remove the offending line (a fresh distinct value is generated) or set a distinct value manually.
- **Stale-secret guard around the local server.** If `qwed init` regenerates a missing secret but a QWED server is already running with older values, the CLI stops before bootstrapping a new API key — issuing one against a server that cannot verify it would produce a key that dies on the next restart.

See [`qwed init`](/advanced/cli#qwed-init-onboarding-and-bootstrap) and [API key storage and migration](/api/authentication#api-key-storage-and-migration-v7-2) for the full contract.

---

## QWED-A2A — `/a2a/intercept` requires per-agent API keys (breaking)

**Released: September 9, 2026** · [GitHub PR #106](https://github.com/qwed-ai/qwed-a2a/pull/106)

> The FastAPI gateway's `POST /a2a/intercept` endpoint previously trusted the body-declared `sender_agent_id` — any caller could impersonate any agent for trust evaluation, rate limits, and attestation. The endpoint now authenticates every request with a per-agent API key in the `X-API-Key` header, validated against the `QWED_A2A_API_KEYS` environment variable. The authenticated identity overrides the body's `sender_agent_id` everywhere it matters. The dead `AgentMessage.signature` field, which was never verified, is removed.

<Warning>
**Fail-closed behavior change.** Deployments that do not set `QWED_A2A_API_KEYS` now deny every `/a2a/intercept` request with `401 API authentication is not configured for this service.` There is no anonymous access and no opt-out. Configure a key for each calling agent before upgrading.
</Warning>

### What changed

- **Per-agent API-key authentication** — `QWED_A2A_API_KEYS` holds a JSON object mapping each key to its agent ID, e.g. `{"key-abc123": "procurement-agent"}`. Missing, unknown, or unconfigured keys return `401`. Keys are compared in constant time.
- **Authenticated identity wins** — trust-boundary evaluation, token-bucket rate limiting, telemetry, and the attestation JWT all bind to the key's agent ID. If the body's `sender_agent_id` differs, the gateway logs a warning and overrides it. Direct library callers of `intercept()` get no override and own identity binding themselves.
- **No-restart key rotation** — the key map is read fresh on every request. Update the environment variable and old keys stop matching immediately.
- **Agent-ID normalization** — map entries are trimmed and validated against the `AgentMessage` contract (max 256 chars, no control characters); invalid entries are dropped and their keys denied.
- **`AgentMessage.signature` removed** — the optional field was documented as "JWT signature for tamper detection" but was never read by any verification path. Attestation integrity comes from the verdict JWT, not a caller-supplied field.

### Migration

Before (now returns `401`):

```bash
curl -X POST http://localhost:8000/a2a/intercept \
  -H "Content-Type: application/json" \
  -d '{"sender_agent_id": "agent-A", "receiver_agent_id": "agent-B", "payload_type": "general", "payload": {"message": "Hello!"}}'
```

After — configure a key map on the gateway and send the header:

```bash
export QWED_A2A_API_KEYS='{"key-for-A": "agent-A"}'
...
If any client still sends `signature` in the message body, remove the field. See [Gateway authentication](/a2a/deployment#gateway-authentication) for the full key format and rotation guidance.

## QWED-A2A — cross-deployment trusted issuers and jti replay lifecycle (security release)

**Released: September 9, 2026** · [GitHub PR #108](https://github.com/QWED-AI/qwed-a2a/pull/108) · [GitHub PR #109](https://github.com/QWED-AI/qwed-a2a/pull/109) · Closes [issue #84](https://github.com/QWED-AI/qwed-a2a/issues/84) and [issue #85](https://github.com/QWED-AI/qwed-a2a/issues/85)

> Two fixes to `A2ACryptoService` attestation verification. Peer deployments can now be registered as trusted issuers, so verifying another deployment's attestations no longer tempts operators into sharing one private key across deployments. And the jti replay registry now separates issuance from consumption: an issuer can verify its own token once, duplicate `trace_id`s are rejected at signing, and the consumption registry is injectable for cross-worker replay protection.

### Cross-deployment verification (PR #108)

- **Trusted issuers.** `verify_attestation` accepts an optional `trusted_issuers` mapping of issuer IDs to `{"deployment_id": ..., "jwks": ...}`, with the `QWED_A2A_TRUSTED_ISSUERS` environment variable as fallback. Register a peer by copying its `/.well-known/jwks.json` entry verbatim. The env var is re-read on every verification, so key rotation needs no restart.
- **Fail-closed on every mismatch.** Unknown issuers, `kid` mismatches, and `deployment_id` mismatches all deny. Routing never trusts unverified claims: header and payload are read only after a candidate key verifies the signature, and one public key registered under two issuer identities keeps only the first identity.
- **Verifier-only nodes.** A node with no local signing key can verify peer tokens against trusted issuers. Only signing, JWKS publishing, and the interceptor still require `QWED_A2A_SIGNING_KEY_PEM`. With neither a local key nor trusted issuers, verification returns `No verification keys available` instead of raising.
- **Peer jti namespacing.** Replay tracking for peer tokens is namespaced per issuer, so two deployments reusing the same trace ID never shadow each other.

### jti registry lifecycle (PR #109)

- **Issuance/consumption split.** `sign_verdict` records the `trace_id` in a separate per-instance issuance record instead of the consumption registry, so an issuer can verify its own token once — the second presentation is a replay.
- **Duplicate trace IDs rejected at signing.** Reusing a `trace_id` within its validity window raises `ValueError` at `sign_verdict` instead of minting two tokens under one `jti`.
- **Injectable consumption registry.** `A2ACryptoService(jti_registry=...)` accepts a shared registry (Redis, database) for cross-worker replay protection. The default remains process-local: two workers in two processes share nothing. Injected registries must provide atomic insert-if-absent, retention covering token lifetime, and a numeric `ttl_seconds` at least `validity_seconds` — anything less raises `ValueError` at construction.

### Upgrading

<Warning>
**Behavior changes.** Code that relied on signing twice with the same `trace_id` now gets `ValueError` at the second `sign_verdict`. Code that verified the same token twice on one instance now gets `Replay detected: jti already seen` on the second call — previously issuers could not self-verify at all, while consumers could sometimes replay. Verifier-only nodes that expected `verify_attestation` to raise `RuntimeError` without a signing key now get a `(False, None, ...)` tuple or, with trusted issuers configured, successful peer verification. Multi-worker deployments should inject a shared `jti_registry` — the default registry does not detect replays across processes.
</Warning>

See [Crypto attestations](/a2a/crypto-attestations#cross-deployment-verification-trusted-issuers) for configuration and the full replay-scope documentation.

---

## v7.2.1 — Math answer comparison security patch

**Released: September 14, 2026** · [GitHub Release ↗](https://github.com/QWED-AI/qwed-verification/releases/tag/v7.2.1) · [GitHub PR #365](https://github.com/QWED-AI/qwed-verification/pull/365) · Advisory [GHSA-xmm6-8r3x-j567](https://github.com/QWED-AI/qwed-verification/security/advisories/GHSA-xmm6-8r3x-j567)

> v7.2.1 is a fail-closed security patch. The math answer comparison path previously fed the untrusted model answer to raw `sympy.sympify`, which is an eval sink that can execute dunder-traversal gadgets smuggled in the answer turn. The answer is now parsed only through the hardened `safe_parse_expr`, and unparseable input fails closed. No new capability and no breaking wire changes. Upgrade is recommended for any deployment on `v6.0.0` through `v7.2.0`.

### Security fix

- **Hardened answer parsing.** `QWEDLocal._math_answers_match` no longer calls `sympy.sympify` on the model's answer. It now parses through `safe_parse_expr`, which applies a character allow-list, an AST allow-list, strips builtins, and enforces the compute-cost bounds introduced in v7.2.0. Unparseable answers and a missing parser both return `False`, so the comparison never treats an attacker-controlled string as "equal" by accident.
- **Release boundary gate covers the SDK.** `scripts/check_boundary.py` now scans `qwed_sdk/` and forbids raw `sympify` outside approved wrappers, so the eval sink cannot reappear in a future refactor.

### Version bumps

| Surface | From | To |
|---|---|---|
| `qwed` (PyPI) | `7.2.0` | `7.2.1` |
| `qwed_sdk` (Python) | `7.2.0` | `7.2.1` |
| `@qwed-ai/sdk` (npm) | `7.2.0` | `7.2.1` |
| `qwed` (crates.io) | `7.2.0` | `7.2.1` |
| API version marker | `7.2.0` | `7.2.1` |

### Upgrading

Patch release with no code changes required. Pin `qwed>=7.2.1` on PyPI, `@qwed-ai/sdk@^7.2.1` on npm, and `qwed = "7.2.1"` on crates.io, then redeploy. Wire formats and public APIs are unchanged.
...
| [#351](https://github.com/QWED-AI/qwed-verification/pull/351) | Sandbox containment: log rotation, pids_limit, container removal, result size caps |
