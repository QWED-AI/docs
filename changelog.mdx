...
**Released: April 4, 2026** · [GitHub PR #121](https://github.com/QWED-AI/qwed-verification/pull/121)

> Major release focused on making QWED's verification boundary fail-closed, deterministic about what it proves, and substantially harder to bypass under adversarial conditions. Consolidates 98 commits and 20 merged PRs since v4.0.1.
...
- **`ActionContext` mandatory for agents** — `verify_action()` requires `ActionContext` with `conversation_id` and `step_number`. Requests without these fields are rejected with `QWED-AGENT-CTX-001`.
- **`INCONCLUSIVE` is now a distinct verification status** — Downstream consumers must handle it. Natural-language math responses return `INCONCLUSIVE` when verifying LLM-translated expressions.
- **`BLOCKED` and `UNKNOWN` are explicit outcomes** — Not generic failures. `verify_identity()` numerical sampling fallback now returns `UNKNOWN` instead of `LIKELY_EQUIVALENT`.
- **`/metrics` and `/metrics/prometheus` require admin auth** — Update monitoring integrations to provide a JWT or API key with `owner` or `admin` role.
- **Docker required for stats and consensus** — In-process execution fallbacks have been removed.
...
- **Logic verifier eval() removed** — Raises `RuntimeError` if `SafeEvaluator` is unavailable instead of falling back to unrestricted `eval()`.
- **Consensus rate limiting** — `POST /verify/consensus` now enforces `check_rate_limit` to prevent cost amplification attacks.
- **Consensus fact self-attestation removed** — Fact engine no longer calls `verify_fact(query, query)` — requires external context.
...
- Heuristic and non-proof outcomes are honestly labeled instead of presented as formal verification.

### Agent hardening

- **Action context mandatory** — `verify_action()` requires `ActionContext` with `conversation_id` and `step_number`.
- **Replay detection** — Same `(conversation_id, step_number)` pair is blocked (`QWED-AGENT-LOOP-002`).
