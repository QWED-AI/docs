...
## QWED-Finance — Release blockers N-01 and N-04 fixed (NO-GO → GO)
**Released: May 2, 2026** · [GitHub PR #24](https://github.com/QWED-AI/qwed-finance/pull/24)

> Resolves the two release-blocking findings from the post-fix deterministic audit. Eliminates the last fail-open execution path in `FinanceVerifier` and the last precision divergence between the integration layer and `DerivativesGuard`.

### What changed

- **N-01 (HIGH) — Integration layer precision parity.** `OpenResponsesIntegration._verify_option_price()` previously carried its own Black-Scholes implementation using `math.log/exp/sqrt/erf` (IEEE-754 float), while `DerivativesGuard.verify_black_scholes()` uses `mpmath` (arbitrary precision). Same formula, two precision paths — same input could produce different output depending on the call route. The duplicate implementation has been removed; `_verify_option_price` now delegates to `DerivativesGuard.verify_black_scholes()`. The tool result's `price` and `delta` are now sourced from `bs_result.computed_price` and `bs_result.greeks["delta"]`.
- **N-04 (MEDIUM) — Compound interest fail-closed.** `FinanceVerifier.verify_compound_interest()` previously used `n_map.get(compounding, 1)`, silently defaulting unknown frequencies (`"weekly"`, `"continuous"`, typos) to annual compounding — a classic fail-open that produced a wrong result with no error signal. The method now raises `ValueError` listing the accepted values: `annual`, `semi-annual`, `quarterly`, `monthly`, `daily`.

### Behavior comparison

| Path | Input | Before | After |
| --- | --- | --- | --- |
| `OpenResponses.price_option` | `S=100, K=105, T=0.25, r=0.05, σ=0.20, type=CALL` | float Black-Scholes (IEEE-754 path) | `DerivativesGuard.verify_black_scholes()` (mpmath, 30 dp) |
| `OpenResponses.price_option` result | — | `{"price": "$3.50", "delta": 0.4502}` (float) | `{"price": "$3.50", "delta": "0.4502"}` (Decimal-quantized strings from the guard) |
| `FinanceVerifier.verify_compound_interest(..., compounding="weekly")` | unknown frequency | Silently used `n=1` (annual) | Raises `ValueError: Unknown compounding frequency: 'weekly'. Accepted values: annual, daily, monthly, quarterly, semi-annual` |
| `FinanceVerifier.verify_compound_interest(..., compounding="annual")` | known frequency | `n=1` | `n=1` (unchanged) |

### Breaking changes

<Warning>
**`verify_compound_interest` raises on unknown compounding values.** Callers that previously relied on the silent annual fallback must either pass one of the accepted values or wrap the call in a `try/except ValueError`. Accepted values: `annual`, `semi-annual`, `quarterly`, `monthly`, `daily`.

**`price_option` `delta` is now a Decimal-quantized string** (sourced from `DerivativesGuard.greeks["delta"]`) instead of a `float`. This matches the rest of the post-mpmath migration. Cast with `Decimal(result["delta"])` if you need numeric arithmetic.
</Warning>
