...
> Tags `qwed-finance` v2.1.0 and rolls up every fix from the deterministic security audit (Issue #16). The release closes all P0/P1 findings across four merged PRs, eliminates IEEE-754 `float` from the financial guards, and makes integration-layer behavior fail-closed and bit-identical to the underlying guards.

### What changed

- **`OpenResponsesIntegration`** — Tools registered without a `verification_fn` are now rejected (previously auto-approved). Built-in tools return the new `ToolCallStatus.COMPUTED` instead of falsely claiming `verified: true`. The `price_option` tool delegates to `DerivativesGuard.verify_black_scholes`, so integration-layer pricing is bit-identical to direct guard pricing. The AML high-risk country list is unified through `ComplianceGuard.high_risk_countries` (14 countries).
- **`BondGuard`** — Newton-Raphson YTM, duration, and convexity run in `Decimal` at 50 dp. `_parse_rate()` no longer guesses format: bare numbers are decimal fractions, percentage inputs require an explicit `%` suffix, and the function returns `Decimal`.
- **`DerivativesGuard`** — Black-Scholes, `_norm_cdf`, `_norm_pdf`, and put/call parity use `mpmath` at 30 dp instead of `math.log/exp/sqrt/erf`. Greeks are returned as `Decimal`-quantized strings.
- **`RiskGuard`** — `Decimal.sqrt()` replaces `math.sqrt()` in VaR and Sortino. Beta accumulates covariance in `Decimal` to prevent catastrophic cancellation.
- **`FinanceVerifier.verify_compound_interest`** — Unknown `compounding` values raise `ValueError` listing accepted frequencies (`annual`, `semi-annual`, `quarterly`, `monthly`, `daily`) instead of silently defaulting to annual.
- **Dependencies** — `mpmath>=1.3.0` is now an explicit runtime dependency (was transitive via `sympy`).
- **Test suite** — 150 tests pass, including 23 float-contamination tests and the N-04 regression for compound-interest fail-closed behavior.

### Breaking changes

<Warning>
**Greeks are now `Decimal`-quantized strings, not `float`.** `DerivativesGuard.verify_black_scholes` and `OpenResponsesIntegration.price_option` both return Greeks as strings. Cast explicitly when arithmetic is needed:
...
<Warning>
**`BondGuard._parse_rate()` returns `Decimal` instead of `float`** and no longer divides bare numbers ≥ 1 by 100. Use an explicit `%` suffix for percentage inputs:

```python
# Before (v2.0.1)
guard._parse_rate("1.5")     # 0.015 (silent heuristic)

# After (v2.1.0)
guard._parse_rate("1.5")     # Decimal("1.5")
guard._parse_rate("1.5%")    # Decimal("0.015")
```
</Warning>

<Warning>
**Unknown compounding frequencies fail closed** in `FinanceVerifier.verify_compound_interest`:

```python
# Before — silent fallback to annual
verifier.verify_compound_interest(
    principal=10000, rate=0.05, periods=10,
    compounding="weekly", llm_amount="$16,288.95",
)

# After — ValueError listing accepted frequencies
verifier.verify_compound_interest(
    principal=10000, rate=0.05, periods=10,
    compounding="weekly", llm_amount="$16,288.95",
)
...
<Warning>
**Unregistered tools in `OpenResponsesIntegration` are now rejected.** Consumers checking `status == APPROVED` must also handle `ToolCallStatus.COMPUTED` as a valid non-error state, and every registered tool must supply a `verification_fn`.
</Warning>
...
| --- | --- | --- |
| [#21](https://github.com/QWED-AI/qwed-finance/pull/21) | S-01, S-02, S-04, S-06 | OpenResponses fail-closed default, `COMPUTED` status, AML country unification |
| [#22](https://github.com/QWED-AI/qwed-finance/pull/22) | C-04, H-01, S-05 | Rate parsing heuristic removed; `_parse_rate` returns `Decimal` |
| [#23](https://github.com/QWED-AI/qwed-finance/pull/23) | C-01, C-02, C-03 | Float → `Decimal`/`mpmath` migration in BondGuard, DerivativesGuard, RiskGuard |
| [#24](https://github.com/QWED-AI/qwed-finance/pull/24) | N-01, N-04 | Integration precision unification; compound-interest fail-closed |
...
GitHub Action consumers should bump the ref:
...
See [Open Responses](/finance/integrations/open-responses), [Derivatives Guard](/finance/guards#3-derivatives-guard-black-scholes), [Bond Guard](/finance/guards#8-bond-guard-yield-analytics), and [Risk Guard](/finance/guards#10-risk-guard-portfolio-metrics) for the updated guard documentation.
