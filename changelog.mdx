...
## v5.0.6 — Schema `uniqueItems` fail-closed
**Released: April 19, 2026** · [GitHub PR #155](https://github.com/QWED-AI/qwed-verification/pull/155)
...
- **`uniqueItems` validation fail-closed** — When a JSON schema specifies `"uniqueItems": true` and the array contains items that cannot be hashed for comparison (e.g., deeply nested or mixed-type structures), the schema verifier previously caught the error silently and skipped the uniqueness check entirely. An attacker could craft array payloads with unhashable items to bypass the duplicate detection. The verifier now emits a `uniqueness_validation_error` issue with the message `"uniqueItems could not be verified deterministically"`, ensuring the validation fails rather than passing by default.

### Response example

When `uniqueItems` validation cannot complete, the schema verifier now returns an issue like:
...
If your schemas use `"uniqueItems": true` with arrays containing unhashable items (objects or lists that cause `TypeError` or `ValueError` during comparison), those arrays now produce a validation error instead of passing silently. Ensure your array items are serializable and comparable, or handle the `uniqueness_validation_error` issue type in your response processing.

---

## v5.0.5 — Ambiguous math expressions fail closed
**Released: April 19, 2026** · [GitHub PR #154](https://github.com/QWED-AI/qwed-verification/pull/154)

> The math verification API now rejects ambiguous expressions with `BLOCKED` instead of returning `is_valid: true`. Expressions like `1/2(3+1)` where implicit multiplication follows division are no longer treated as valid.

### Security

- **Ambiguous math expressions fail closed** — When `POST /verify/math` detects an expression with implicit multiplication after division (e.g., `1/2(3+1)`), the parsing is ambiguous because it could mean `(1/2)*(3+1)` or `1/(2*(3+1))`. Previously, the endpoint returned `is_valid: true` with a `warning: "ambiguous"` field, which downstream code could interpret as a successful verification. The endpoint now returns `is_valid: false`, `result: false`, and `status: "BLOCKED"`, making the rejection explicit. The `warning: "ambiguous"` and `message` fields are still included to explain why the expression was blocked.
...
If your code submits expressions with implicit multiplication after division (e.g., `1/2x`, `6/3(2+1)`) and relies on `is_valid: true`, those requests now return `is_valid: false` with `status: "BLOCKED"`. Rewrite the expression with explicit parentheses or an explicit `*` operator to remove the ambiguity.
...
> Identity verification now fails closed when only numerical sampling matches. The engine returns `BLOCKED` with `is_equivalent: false` instead of `UNKNOWN` with `is_equivalent: null`.

### Security

- **`verify_identity` fail-closed** — When `verify_identity` cannot establish a formal proof but all numerical sample points agree, it previously returned `UNKNOWN` with `is_equivalent: null`. A crafted expression could match at the fixed sample points without being algebraically equivalent, and downstream code treating `null` as non-failure could misinterpret the result. The engine now returns `BLOCKED` with `is_equivalent: false`, `method: "numerical_sampling_rejected"`, and `confidence: 0.0`, making the rejection explicit and unambiguous.

### Impact

No migration is required unless your code checks for `status: "UNKNOWN"` or `is_equivalent: null` from identity verification. Update those checks to handle `status: "BLOCKED"` and `is_equivalent: false` instead.
