...
- **Empty body** on a protected path and method now returns `422` with `"Empty request body: cannot verify empty payload"`.
- **Malformed or non-UTF-8 body** returns `422` with `"Malformed request body: expected JSON"`.
- **Top-level non-object JSON** (array, number, string, `null`, boolean) returns `422` with `"Invalid request body: expected JSON object"`.
