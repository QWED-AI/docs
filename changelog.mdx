...
> Closes a fail-open gap in `AgentService.verify_action`. Previously, an `action_type` with no explicit registered semantics could pass risk assessment and verification, then receive an `APPROVED` decision with a generic `"security"` engine fallback. The runtime now denies unknown action types before any risk or verification checks run.

### What changed

- **Unknown `action_type` is denied with `QWED-AGENT-ACTION-001`.** Action types that are not registered in either the action-engine map or the tool risk map are now rejected before risk assessment, with no `verification` block in the response.
- **Generic `"security"` engine fallback removed.** `verification.engine` now always reflects the explicitly registered engine (or `"tool_control"` for registered tool actions). There is no implicit catch-all engine label.
- **In-flight step reservation released on denial.** The denied step number is freed so the agent can retry with a registered action on the same step.
- **Explicit tool actions keep their deterministic risk path.** Registered actions continue to resolve through `ACTION_ENGINES` or `TOOL_RISK_LEVELS` unchanged.
...
- **Unknown `action_type` values are denied** with the new error code `QWED-AGENT-ACTION-001`. The denial happens before risk assessment, so unknown actions can never be marked `VERIFIED`.
- **Generic `"security"` engine fallback removed** from the verification response. The `engine` field now only reflects an explicitly registered engine (e.g. `sql`, `code`, `math`, `tool_control`).
- **In-flight step reservation released on denial.** Agents can retry the same `step_number` with a registered action without triggering replay detection (`QWED-AGENT-LOOP-002`).
- **Explicitly registered tool actions remain on their existing deterministic risk path** — no behavior change for known engines or tools.

### Migration

If you call `verify_action` with custom `action_type` strings, route them through one of the registered verification engines or tool calls. See [agent verification — fail-closed on unknown actions](/advanced/agent-verification#fail-closed-on-unknown-actions) for the full registered list.
