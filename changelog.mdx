...
> `VerificationCache` now keys every entry on both the normalized query **and** the trust-bound context (provider, model, policy version, tenant/session, environment fingerprint). A mismatch on any context dimension is a deterministic cache miss, preventing cross-context replay of `VERIFIED` results.
...
- **Composite cache key** — Entries are now keyed by `SHA-256(normalized_query)` combined with `SHA-256(canonical_context_json)`. Identical queries stored under different contexts never collide.
- **New `cache_v2` schema** — The on-disk SQLite table uses a composite `PRIMARY KEY (key, context_fingerprint)` and stores the context fingerprint per entry. The fingerprint is re-validated on every read as defence-in-depth.
- **Legacy entries are never returned** — Pre-v2 cache rows (without a context fingerprint) are unreachable from any code path and are treated as dead weight.
...
- Cached results created under one (provider, model, policy version, tenant) tuple will not be returned to a caller using a different tuple — even if the query string is identical.
