...
<Warning>
Deployments that scrape `/metrics` (e.g., Prometheus) will start receiving 401/403 after upgrading. Create a dedicated operator account, mint it an API key, and add its user ID to `QWED_METRICS_OPERATOR_USER_IDS` before deploying. The bundled Prometheus configuration in `deploy/` documents the bootstrap steps. See [Security Hardening](/advanced/security-hardening#all-tenant-metrics-authorization).
</Warning>
