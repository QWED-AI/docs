...
<Warning>
**Behavior change.** Inputs that previously received `VERIFIED` may now receive `BLOCKED` or `CORRECTION_NEEDED` — treat both as unverified. If your code branched on `result.verified` or `result.status == "VERIFIED"` alone, it will continue to work; if you consumed `calculated`/`calculated_irr`/`calculated_eigenvalues` from a non-`VERIFIED` result as a fallback, that path is now closed. Consume the new response fields (`ambiguous_modes`, `calculated_count`/`claimed_count`, `converged`, `iterations_used`) instead.
</Warning>
