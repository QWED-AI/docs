...
description: "Archived release notes for older QWED Protocol updates: past guard fail-closed hardening, breaking changes, and SDK migrations across QWED engines."
---

Older changelog entries. See [Changelog](/changelog) for recent releases.
...
See [Agent verification — Unknown action types denied](/advanced/agent-verification#fail-closed-for-unknown-action-types) and the [Agent spec error code table](/specs/agent#appendix-a-error-codes).
...
See the per-PR entries below for full implementation detail: [N-01/N-04 release blockers](#qwed-finance-—-release-blockers-n-01-and-n-04-resolved-no-go-→-go), [Decimal/mpmath migration](#qwed-finance-—-decimal%2Fmpmath-migration-for-bondguard-derivativesguard-and-riskguard), [OpenResponses fail-closed](#qwed-finance-—-openresponses-fail-closed-and-computed-status), and [rate parsing heuristic removed](#qwed-finance-—-rate-parsing-heuristic-removed).
...
See the [`POST /verify/natural_language`](/api/endpoints#post-%2Fverify%2Fnatural_language) endpoint reference for full field descriptions.
