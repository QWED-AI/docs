...
<Warning>
Numerical sampling cannot prove equivalence. If all sample points agree but no formal proof was established, the engine **fails closed** — it returns `BLOCKED` with `is_equivalent: false` and `confidence: 0.0` instead of claiming the expressions are equivalent. This prevents crafted expressions that happen to match at the fixed sample points from passing verification. Treat `BLOCKED` results as unverified.
</Warning>
...
| Ambiguous expressions | Returns "BLOCKED" — implicit multiplication after division is rejected |
