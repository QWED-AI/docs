...
Identity verification uses symbolic simplification first. If symbolic methods cannot establish equivalence, the engine samples five test points as a numerical fallback. Only points that can be successfully evaluated count toward agreement — domain-restricted expressions (e.g., `log(x)` at `x = -1`) are skipped rather than causing a false negative.

<Warning>
When the numerical sampling fallback is used, the result status is `UNKNOWN` with `confidence: 0.0` and `is_equivalent: null`. Matching at fixed sample points does not constitute a formal proof of equivalence. This change was introduced to prevent over-claiming determinism on results that are inherently heuristic.
</Warning>
