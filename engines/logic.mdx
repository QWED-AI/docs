...
<Warning>
The legacy `VerificationEngine.verify_logic_rule()` method has been removed as of v5.0.3. It now raises `NotImplementedError`. Use `LogicVerifier` from `qwed_new.core.logic_verifier` or the SDK's `client.verify_logic()` method instead. See the [changelog](/changelog#v503--verify_logic_rule-fail-closed) for migration details.
</Warning>
