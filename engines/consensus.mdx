...
When consensus verification includes the Python engine (in `high` or `maximum` mode), all generated Python code runs through the secure Docker sandbox via `SecureCodeExecutor`. The engine no longer uses the in-process `CodeExecutor`.

If Docker is unavailable, the Python engine records a `SECURE_EXECUTION_REQUIRED` error, and the consensus calculation immediately returns a `blocked_secure_execution` status — the API surfaces this as HTTP 503.
...
<Note>
The Fact engine is no longer selected during automatic consensus verification. It requires external fact context to produce meaningful results. If you need fact-based verification, call the [Fact engine](/engines/fact) directly.
</Note>
