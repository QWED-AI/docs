...
The Code Engine detects `curl` or `wget` commands piped into a shell through `subprocess`. Both regex-based and AST-based checks are used, so the detection works even when the call spans multiple lines or uses f-strings.
...
Patterns that trigger this detection include:

- `subprocess.run("curl ... | bash", shell=True)` — direct shell execution
- `subprocess.Popen(["wget", "...", "|", "sh"])` — list-form invocation with pipe operators
- Multi-line `subprocess.call(...)` with `curl`/`wget` and `bash`/`sh` in the command string
