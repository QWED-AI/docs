...
sidebar_position: "4"
title: "Code engine"
description: "QWED Code engine reference: CrossHair symbolic execution for Python contracts, safety checks, and bounded model checking with DiagnosticResult output."
---
...
Separate from symbolic execution, `CodeVerifier` scans code for security vulnerabilities using AST and pattern analysis. It backs [`POST /verify/code`](/api/endpoints#post-verifycode) and the `CODE` item type in batch verification. As of v7.0.0, `verify_code()`, `verify_python_deep()`, and `verify_batch()` return a `DiagnosticResult`.
