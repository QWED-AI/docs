...
<Info>Updated in v5.1.0: The legacy `CodeExecutor` class is now hard-blocked. All code execution uses `SecureCodeExecutor` exclusively. See the [changelog](/changelog#v5-1-0-—-agent-state-governance-and-fail-closed-hardening) for migration details.</Info>
