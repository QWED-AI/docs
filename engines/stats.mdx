...
| Condition | Error |
| --------- | ----- |
| Empty series (zero rows) | `{operation} is undefined for an empty series` |
| All values are NaN | `{operation} is undefined because the series has no valid non-NaN values` |
| Multiple modes (multimodal data) | `mode is ambiguous because {n} equally frequent values exist` |
| Result is NaN | `{operation} produced an undefined result (NaN)` |

```python
import pandas as pd

# Empty series — returns ERROR
