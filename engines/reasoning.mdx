...
<ParamField path="reasoning trace" type="required">
  A substantive reasoning trace must be produced by the primary provider. The verifier rejects empty traces and traces that begin with placeholder or failure markers, including:
  ...
  When any of these markers appear, the result includes the issue `Reasoning trace unavailable or non-substantive` and `is_valid` is `False`.
</ParamField>
