...
<Card title="v5.0.0 — Enforcement Boundary Hardening" icon="shield-check" href="/changelog">
  Major release — fail-closed verification, deterministic trust boundaries, agent hardening, and SDK version bump to 5.0.0.
</Card>
...
```bash cargo
cargo add qwed@5.0.0
```

```bash go
go get github.com/QWED-AI/qwed-verification/sdk-go@v5.0.0
```
...
<Accordion title="v5.0.0 — Enforcement Boundary Hardening (April 4, 2026)" icon="shield-check" defaultOpen>
  **98 commits** · Fail-closed verification boundary · `security_checks` removed (server-enforced) · `ActionContext` mandatory for agents · `INCONCLUSIVE`/`BLOCKED`/`UNKNOWN` as explicit statuses · `trust_boundary` metadata · `/metrics` admin auth · Redis fail-closed rate limiting · Agent replay/loop detection · SDK `5.0.0` across Python, TypeScript, Go.

  [Full Release Notes →](/changelog) · [GitHub PR ↗](https://github.com/QWED-AI/qwed-verification/pull/121)
</Accordion>
