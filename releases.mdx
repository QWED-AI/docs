...
<Card title="v6.0.0 — Trust Boundary Completion" icon="shield-check" href="/changelog#v6-0-0--trust-boundary-completion">
  Trust Boundary Completion epic closed (Issue #263, 12/12). Every `/verify/*` endpoint returns unified `DiagnosticResult`. Control plane requires and verifies attestation before admitting VERIFIED. VERIFIED is a protocol guarantee backed by deterministic `proof_ref` — never by execution, agreement, or confidence. Breaking API response schema.
</Card>
...
```bash npm
npm install @qwed-ai/sdk@6.0.0
```

```bash cargo
cargo add qwed@6.0.0
```

```bash go
go get github.com/QWED-AI/qwed-verification/sdk-go@v6.0.0
```
...
<Accordion title="v6.0.0 — Trust Boundary Completion (August 2, 2026)" icon="shield-check" defaultOpen>
