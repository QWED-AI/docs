...
<Card title="v5.0.0 — Enforcement Boundary Hardening" icon="shield-check" href="/changelog">
  Major release — fail-closed verification boundary, trust boundary metadata, mandatory agent context, and consolidated security hardening.
</Card>
...
<Accordion title="v5.0.0 — Enforcement Boundary Hardening (April 4, 2026)" icon="shield-check" defaultOpen>
  Fail-closed verification boundary · `INCONCLUSIVE` status for LLM-translated math · `trust_boundary` metadata in responses · Mandatory `ActionContext` for agents · Replay/loop detection · Redis fail-closed rate limiting · Docker required for stats/consensus · `security_checks` field removed · Admin-only `/metrics`.
