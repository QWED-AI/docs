...
<Card title="v7.1.0 — Verification Context v1.0 Rollout" icon="shield-check" href="/changelog#v7-1-0-%E2%80%94-verification-context-v10-rollout">
  Verification Context (VC) v1.0 shipped end-to-end: a formal spec with JSON Schema, public `proof_ref` generation and resolution, `to_verification_context()` on all 13 verifiers, API endpoints, CLI commands, SDK re-exports, and Docker-action VC outputs. Additive minor — no breaking wire changes.
</Card>
...
```bash pip
pip install qwed==7.1.0
```

```bash docker
docker pull qwedai/qwed-verification:7.1.0
```

```bash npm
npm install @qwed-ai/sdk@7.1.0
```

```bash cargo
cargo add qwed@7.1.0
```

```bash go
go get github.com/QWED-AI/qwed-verification/sdk-go@v7.1.0
```
...
<Accordion title="v7.1.0 — Verification Context v1.0 Rollout (August 16, 2026)" icon="shield-check" defaultOpen>
