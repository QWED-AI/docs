...
print(result.verified)                           # True on success
print(result.details["attestation_id"])          # UUID — matches JWT `jti`
print(result.details["checkout_hash"])           # SHA-256 of checkout
```
...
receipt = guard.create_receipt(
    checkout=checkout,
    verification_result=verification,
    attestation_id=result.details["attestation_id"],
    transaction_attempt_id=transaction_attempt_id,
