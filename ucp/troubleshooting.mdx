...
| "Empty request body" | Middleware | No body on protected path | Send a JSON object body |
| "Malformed request body: expected JSON" | Middleware | Non-JSON or non-UTF-8 body | Use `Content-Type: application/json` |
| "Invalid request body: expected JSON object" | Middleware | Top-level array/primitive | Wrap payload in an object |
