...
sidebar_position: 2
title: The 9 Guards
description: QWED Legal guard reference for deadline checks, liability math, clause contradiction detection, citation validation, jurisdiction checks, limitation-period analysis, IRAC reasoning, fairness testing, and Z3-powered logic verification.
---
...
1. **Counterfactual Generation** - Swaps protected attributes (names, pronouns) while preserving case
2. **Re-evaluation** - Runs the modified prompt through the LLM
3. **Deterministic Comparison** - Checks if outcomes match exactly
...
| `NO_SWAP_REQUIRED`       | No protected attributes to test                |
