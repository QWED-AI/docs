...
Each entry in `results` is a per-item response. Math items are returned as [`DiagnosticResult`](/advanced/diagnostics) records — the same three-layer verdict shape used by `POST /verify/math` — so batch and single-item math verifications share one contract:

- `status` — `VERIFIED`, `UNVERIFIABLE`, or `BLOCKED`.
- `agent_message` — short human-readable summary safe to surface to the caller.
- `developer_fields` — engine detail (including the legacy `is_valid` flag for backward compatibility).
- `proof_ref` — cryptographic hash of the retained proof artifact. Present only when `status == VERIFIED`; `null` otherwise. **This is the authority bit** — downstream gates must reject any item whose `proof_ref` is `null`.
...
<Info>
The pre-v6.0 top-level `is_valid` field is preserved inside `developer_fields.is_valid` for backward compatibility, but new code should gate on `status == "VERIFIED"` and the presence of `proof_ref`. The previous `summary.success_rate` aggregate has been removed for math items because a boolean pass/fail cannot represent the three-status verdict — count `status == "VERIFIED"` entries directly if you need a success ratio.
</Info>
