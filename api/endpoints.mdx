...
The response reports three independent checks: payload hash, HMAC signature (compared in constant time), and chain linkage to the prior entry. Genesis entries must have a `null` `previous_hash`; non-genesis entries must link to a non-empty prior entry hash.
