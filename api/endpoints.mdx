...
Each `math` item in the batch is evaluated according to whether it expresses an equality or proof claim:

| Input shape | Example | Result |
|-------------|---------|--------|
| Equality `lhs = rhs` | `x**2 + 2*x + 1 = (x+1)**2` | `is_valid: true` if `simplify(lhs - rhs) == 0`, otherwise `is_valid: false` with `message: "Not equal"`. |
| Bare expression (no `=`) | `x**2 + 2*x + 1` | `is_valid: false`, `status: "SIMPLIFIED"`, with the simplified form returned in `simplified`. The expression is not treated as a proof. |

A bare expression returns the following result object:
...
<Note>To assert mathematical correctness, submit an equality (`lhs = rhs`). A bare expression is simplified for convenience but is never reported as verified.</Note>
