...
Security checks are enforced server-side on every request — `ExfiltrationGuard` always runs, and `MCPPoisonGuard` runs automatically when a `tool_schema` is provided. You do not need to opt in.

<Warning>
**Breaking change (v4.0.3):** The `security_checks` request field has been removed. Exfiltration and MCP poison checks are now enforced server-side and cannot be disabled by clients. Remove any `security_checks` fields from your request payloads.
</Warning>
...
| `tool_schema` | object | No | MCP tool definition — when provided, `MCPPoisonGuard` runs automatically |
