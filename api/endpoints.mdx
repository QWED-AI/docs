...
The response follows the `VerificationResult` schema. The top-level `status` reflects trust boundary alignment — see the `trust_boundary` object for a detailed breakdown of what was and was not verified.
...
Returns global system metrics and per-tenant breakdowns. Requires admin authentication — you must provide either a JWT token for a user with the `owner` or `admin` role, or an API key linked to an admin user.
...
| 403 | Authenticated but not an admin or owner |
