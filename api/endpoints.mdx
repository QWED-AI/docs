...
For items with `type: "math"`, the result depends on whether the query contains an equality (`=`) or proof claim:

| `is_valid` | `status` | When | Example query |
|------------|----------|------|---------------|
| `true` | `"VERIFIED"` | The two sides of an equality reduce to the same canonical form | `x**2 + 2*x + 1 = (x+1)**2` |
| `false` | `"NOT_EQUAL"` | An equality was provided but the two sides are not equivalent | `x + x = x` |
| `false` | `"SIMPLIFIED"` | No equality or proof claim was provided. The expression is simplified and returned in `simplified`, but no claim was verified | `x + x` |

A `SIMPLIFIED` result is not a verification. Always pass an explicit equality or proof claim if you need a pass/fail outcome. Example simplification-only response item:
