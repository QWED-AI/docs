...
Returns global system metrics and per-tenant breakdowns. Requires admin authentication via Bearer token or API key.

<Warning>**Breaking change (v4.1.0):** The `/metrics` endpoint now enforces admin authentication. Requests from non-admin users or inactive accounts return HTTP 403. Both Bearer token and API key authentication are supported — when using an API key, the associated user must have the `admin` role.</Warning>
...
| Status | Description |
|--------|-------------|
| 403 | Admin access required — user must have `admin` role and be active |
...
Returns metrics in Prometheus text format for scraping by monitoring infrastructure. Requires admin authentication.

<Warning>**Breaking change (v4.1.0):** The `/metrics/prometheus` endpoint now enforces admin authentication, matching the `/metrics` endpoint behavior.</Warning>
