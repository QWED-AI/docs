...
<Info>New in v5.0.0: `verify_identity()` numerical sampling fallback now returns `UNKNOWN` instead of `LIKELY_EQUIVALENT`.</Info>
...
**Ambiguous expressions:** Expressions with implicit multiplication after division (e.g., `1/2(3+1)`) are blocked rather than evaluated, because the intended order of operations is ambiguous. The engine fails closed with `is_valid: false`, `result: false`, and `status: "BLOCKED"`.

**Response (ambiguous expression):**
