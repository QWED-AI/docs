...
Glass-box reasoning process verifier. Checks IRAC structural compliance or validates custom milestone completion rates with decimal scoring.
...
```json
{
  "trace": "Risk assessment complete. Compliance verified. Implementation planned.",
  "mode": "milestones",
  "milestones": ["risk assessment", "compliance", "implementation"]
}
```
...
| `mode` | string | No | `irac` | Verification mode: `irac` or `milestones` |
| `milestones` | array | Conditional | — | List of required milestone strings (required when `mode` is `milestones`) |
...
Document-level retrieval mismatch defender. Verifies that retrieved RAG chunks originate from the expected source document, preventing DRM hallucinations in RAG pipelines.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `target_document_id` | string | Yes | — | Expected source document ID (non-empty) |
| `chunks` | array | Yes | — | Array of chunk objects with metadata (non-empty) |
...
| Status | Description |
|--------|-------------|
| 400 | Invalid payload — empty `target_document_id`, empty `chunks`, or `max_drm_rate` not a valid `Fraction` string between 0 and 1 |
