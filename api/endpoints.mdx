...
An API key resolves through its owning user, so the key must belong to an allowlisted, active user. Expired or revoked keys are not usable credentials; if a request carries both an API key and a JWT, the JWT can still authorize the read.
