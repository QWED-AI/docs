...
The response follows the `VerificationResult` schema:
...
| 503 | Secure execution runtime unavailable for the requested consensus depth (Docker is not running) |

The `503` response occurs when the `high` or `maximum` mode requires the Python engine but the secure Docker sandbox is unreachable.
...
| Status | Description |
|--------|-------------|
| 403 | Verification blocked by security policy |
| 503 | Secure execution runtime unavailable (Docker is not running) |
...
Returns global system metrics and per-tenant breakdowns. In production, this should require admin authentication.
...
Returns metrics in Prometheus text format for scraping by monitoring infrastructure.
