...
All verification requests require an API key. The health endpoints (`GET /health`, `GET /health/identity`) are unauthenticated by design:
...
| GET | `/health/identity` | Server identity handshake for `qwed init` (no auth required) |
