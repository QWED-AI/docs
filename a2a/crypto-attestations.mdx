...
<Tip>
Rotate keys by deploying a new `QWED_A2A_SIGNING_KEY_PEM` across all replicas. The `kid` on newly issued tokens changes automatically; tokens signed by the previous key remain verifiable until they expire. See [Deployment](/a2a/deployment) for the JWKS endpoint and rollout guidance.
</Tip>
