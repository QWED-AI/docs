...
description: "Install QWED A2A, configure a persistent signing key, and run your first zero-trust agent-to-agent verification in about five minutes."
icon: "rocket"
---
...
QWED A2A includes a ready-to-use HTTP gateway. Every call to `POST /a2a/intercept` must authenticate with a per-agent API key in the `X-API-Key` header. Configure keys through the `QWED_A2A_API_KEYS` environment variable, a JSON object mapping each key to its agent ID. There is no anonymous access: if no keys are configured, the endpoint denies every request with `401`.
...
# Run with:
# QWED_A2A_API_KEYS='{"key-for-A": "agent-A"}' \
...
<Warning>
The `X-API-Key` header selects the caller. The gateway processes and attests the message as the key's agent even if `sender_agent_id` in the body names someone else. Missing, unknown, or unconfigured keys return `401`. See [Gateway authentication](/a2a/deployment#gateway-authentication) for key format and rotation.
</Warning>
