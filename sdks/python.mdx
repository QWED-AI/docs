...
`qwed_sdk.cache.VerificationCache` is the persistent SQLite-backed cache used to speed up repeated verifications and reduce LLM cost. Cache entries are **context-bound**: a hit requires an exact match of both the normalized query and the trust-bound `CacheContext`.
...
A call to `cache.get(query, context)` returns `None` when any of the following is true:
...
- The entry was written under the legacy v1 schema (legacy rows are never returned).
