...
Verify multiple items at once. Each entry in `results` is a per-item [`DiagnosticResult`](/advanced/diagnostics) — check `status` and `proof_ref` per item rather than relying on an aggregate.
...
for item in response.results:
    if item["status"] == "VERIFIED" and item["proof_ref"]:
        # Authoritative — admissible for control flow
        print("verified:", item["agent_message"], item["proof_ref"])
    else:
        # UNVERIFIABLE or BLOCKED — must reject
        print("rejected:", item["status"], item["agent_message"])
```
