...
`qwed_sdk.cache` provides a context-bound SQLite cache for verification results. Cache hits require an exact match of both the normalized query **and** the trust-bound `CacheContext`. A mismatch on any context dimension is a deterministic miss — the cache never returns a result across trust boundaries. Legacy entries stored without a context fingerprint are never returned.
...
<ParamField path="ttl" type="int" default="86400">
  Time-to-live in seconds (default: 24 hours). Entries older than `ttl` are treated as misses and deleted on access.
</ParamField>

#### `get(query, context)`

Return the cached result for `(query, context)`, or `None` on miss. A miss is returned when no entry exists for the pair, when the entry has expired, or when the stored context fingerprint does not match `context` (defence-in-depth replay guard).
...
Store a verification result bound to `(query, context)`. Identical queries stored under different contexts are kept as independent entries (composite primary key `(key, context_fingerprint)`).
...
<ParamField path="result" type="Dict[str, Any]" required>
  Verification result to cache. Serialized as JSON.
</ParamField>
...
<Note>
**Breaking change (Issue #187).** `get()` and `set()` now require a `CacheContext` argument. Callers that previously invoked `cache.get("2+2")` or `cache.set("2+2", result)` raise `TypeError`. The on-disk schema is `cache_v2` with composite `PRIMARY KEY (key, context_fingerprint)`; entries in the legacy `cache` (v1) table are never returned.
</Note>
