...
The TypeScript SDK supports the QWED Agent API for registering AI agents and verifying their actions with per-tenant rate limiting and optional security checks.
...
Verify an agent action with optional exfiltration and MCP poisoning checks.
...
| `options.checkExfiltration` | `boolean` | — | Enable exfiltration detection |
| `options.checkMcpPoison` | `boolean` | — | Enable MCP poisoning detection |
| `options.toolSchema` | `object` | — | MCP tool schema to scan |
...
enum VerificationType {
  ...
  Image = 'image',
  Reasoning = 'reasoning',
  Process = 'process',   // New in v4.0.1
  RAG = 'rag',           // New in v4.0.1
  Security = 'security', // New in v4.0.1
}
```
