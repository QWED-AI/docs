...
Verify multiple items at once. Each entry in `results` is a per-item [`DiagnosticResult`](/advanced/diagnostics) with `status`, `agent_message`, `developer_fields`, and `proof_ref` — inspect each item individually instead of relying on an aggregate success rate.
...
for (const item of response.results) {
  if (item.status === 'VERIFIED' && item.proof_ref) {
    // Authoritative — admissible for control flow
    console.log('verified:', item.agent_message, item.proof_ref);
  } else {
    // UNVERIFIABLE or BLOCKED — must reject
    console.log('rejected:', item.status, item.agent_message);
  }
}
```
