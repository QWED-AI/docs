...
Verify an agent action. Exfiltration detection runs on every request. MCP poison inspection runs automatically when you provide a `toolSchema`.

<Warning>
**Breaking change (v4.0.3):** The `checkExfiltration` and `checkMcpPoison` options have been removed. Security checks are now enforced server-side and cannot be disabled. Remove these options from your code.
</Warning>
