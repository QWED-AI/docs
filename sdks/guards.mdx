---
title: "Agentic security guards for the QWED Python SDK"
...
<Note>Equivalent Unicode text produces identical verification results. Payloads are canonicalized to Unicode Normalization Form C (NFC), so precomposed characters like `"\u00C9"` and their decomposed equivalents like `"E\u0301"` yield the same `normalized_state` and the same `proof_ref`.</Note>
