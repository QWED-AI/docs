...
unknown = guard.get_limitation_period("breach_of_contract", "Mars")
print(unknown)  # None
```

### Compare jurisdictions

Unknown jurisdictions are returned as `None` rather than being silently filled with a default period:

```python
comparison = guard.compare_jurisdictions(
    "breach_of_contract",
    ["California", "New York", "Delaware", "Mars"]
)
# {'California': 4.0, 'New York': 6.0, 'Delaware': 3.0, 'Mars': None}
```
