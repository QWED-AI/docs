...
`verify(...)` returns a `StatuteResult` with the following fields:

<ResponseField name="verified" type="bool">
  `True` only when the jurisdiction and claim type are both recognized, the dates parse successfully, and the filing date is on or before the computed expiration date. `False` in every other case, including when the input is unverifiable.
</ResponseField>

<ResponseField name="claim_type" type="str">
  Echo of the claim type passed in.
</ResponseField>

<ResponseField name="jurisdiction" type="str">
  Echo of the jurisdiction passed in.
</ResponseField>

<ResponseField name="incident_date" type="datetime | None">
  Parsed incident date. `None` if the input date could not be parsed.
</ResponseField>

<ResponseField name="filing_date" type="datetime | None">
  Parsed filing date. `None` if the input date could not be parsed.
</ResponseField>

<ResponseField name="limitation_period_years" type="float | None">
  Limitation period applied. `None` when the jurisdiction or claim type is not recognized (no default fallback is used).
</ResponseField>

<ResponseField name="expiration_date" type="datetime | None">
  Computed expiration date. `None` when the limitation period cannot be determined.
</ResponseField>

<ResponseField name="days_remaining" type="int | None">
  Days between filing and expiration (negative if expired). `None` when the limitation period cannot be determined.
</ResponseField>

<ResponseField name="message" type="str">
  Human-readable explanation. Unverifiable inputs return a message prefixed with `⚠️ UNVERIFIABLE` and list the supported jurisdictions or claim types.
</ResponseField>

<ResponseField name="jurisdiction_matched" type="bool">
  `True` if the jurisdiction exactly matched the supported list (case-insensitive). `False` for unknown jurisdictions. Partial-string matching is no longer applied.
</ResponseField>

<ResponseField name="claim_type_matched" type="bool">
  `True` if the claim type was found in the matched jurisdiction's table. `False` for unknown claim types — no default fallback table is consulted.
</ResponseField>

### Fail-closed behavior

`StatuteOfLimitationsGuard` fails closed when it cannot verify an input deterministically:

- **Unknown jurisdiction** — Returns `verified=False`, `jurisdiction_matched=False`, `claim_type_matched=False`, and nullifies all period fields. The message lists the supported jurisdictions.
- **Unknown claim type** (for a recognized jurisdiction) — Returns `verified=False`, `jurisdiction_matched=True`, `claim_type_matched=False`, and nullifies the period fields. The message lists the claim types supported in that jurisdiction.
- **Unparseable dates** — Returns `verified=False`, both match flags `False`, and all date and period fields `None`.

The guard no longer applies a partial-string match on jurisdictions and no longer falls back to a default limitation period when the claim type is unknown. Inputs outside the supported tables must be handled explicitly by the caller.
...
# Returns None for unsupported jurisdictions or claim types
guard.get_limitation_period("fraud", "Atlantis")        # None
guard.get_limitation_period("space_law", "California")  # None
```
