...
`StatuteOfLimitationsGuard` only returns a limitation period when both the jurisdiction and the claim type **exactly** match an entry in its rule tables. Partial string matching and default fallback periods have been removed — the guard never fabricates a limitation period for inputs it does not recognize.

When the guard cannot verify an input, it returns a `StatuteResult` where:

- `verified` is `False`
- `limitation_period_years`, `expiration_date`, and `days_remaining` are `None`
- `message` starts with `⚠️ UNVERIFIABLE` and lists the supported values
- `jurisdiction_matched` and/or `claim_type_matched` are `False`
...
print(result.message)
# ⚠️ UNVERIFIABLE: Jurisdiction 'Mars Colony' is not in the supported
# jurisdiction list. Cannot determine applicable statute of limitations.
# Supported: AUSTRALIA, CALIFORNIA, CANADA, DELAWARE, ...
```

Use the `jurisdiction_matched` and `claim_type_matched` flags to distinguish an unverifiable input from a valid claim that simply expired (where both flags are `True` and `verified` is `False` because the filing date is past `expiration_date`).

<Warning>
  Inputs such as `"CALIF"`, `"NEW"`, or `"CALIFORNIA STATE"` no longer partial-match a supported jurisdiction. Pass exact names such as `"California"` or `"New York"`.
</Warning>
...
Look up the limitation period for a specific claim type and jurisdiction without performing a full verification. Returns `None` when the jurisdiction or claim type is unknown:

```python
years = guard.get_limitation_period("fraud", "Germany")
print(years)  # 10.0

unknown = guard.get_limitation_period("antitrust", "California")
print(unknown)  # None - claim type not modeled
```

### Compare jurisdictions

`compare_jurisdictions` returns `None` for any jurisdiction that is not in the supported list, so callers can distinguish modeled jurisdictions from unknown ones:

```python
comparison = guard.compare_jurisdictions(
    "breach_of_contract",
    ["California", "New York", "Delaware", "Mars Colony"]
)
# {'California': 4.0, 'New York': 6.0, 'Delaware': 3.0, 'Mars Colony': None}
```
