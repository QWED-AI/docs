...
| `difference_days` | `int \| None` | Absolute difference in days between claimed and computed. `None` when `computed_deadline` is `None`. |
| `message` | `str` | Human-readable verification message. Prefixed with `⚠️ UNVERIFIABLE` when the term cannot be parsed. |
| `is_computable` | `bool` | `False` when the term is ambiguous (no numeric quantity, no recognized unit, or unparseable dates); `True` otherwise. |
...
| **Fail-closed** | Returns `is_computable=False` for ambiguous terms instead of fabricating a default |
