...
Recognized time units are `day`/`calendar`, `week`, `month`, and `year`, with optional `business`/`working`/`work` qualifiers for business-day arithmetic. Business/working qualifiers are only valid on `day` units — `"6 business months"` fails closed rather than silently computing a calendar period.
...
Each encoded clause records a `value_provenance` tag in its `verification_trace` step so callers can tell text-parsed values apart from caller-asserted numbers:

| `value_provenance` | Meaning |
| --- | --- |
| `parsed_from_text` | The numeric operand was extracted from `clause.text` (e.g., `"capped at 10000"` → `10000`). Encoded step is `DETERMINISTIC`. |
| `caller_asserted` | The operand came from `clause.value` only; the text did not evidence it. Encoded step is `INFERRED`. |

A `sat` result over clauses whose values are only caller-asserted is reported as `partial_coverage` with `verified=False`. This prevents `Clause(text="exactly 1 month", value=999)` from encoding `var==999` and having Z3 "prove" consistency of an invented number. The result message notes how many values were caller-asserted.
