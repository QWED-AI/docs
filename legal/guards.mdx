...
`verify_cap`, `verify_indemnity_limit`, and `verify_tiered_liability` reject inputs that cannot be safely quantized. All numeric fields on `LiabilityResult` and `TieredLiabilityResult` are now `Optional[Decimal]` — the guard returns `None` for those fields instead of fabricating zeros when it cannot compute. (Unreleased on `main`, post-`0.4.0` — on published `0.4.0` these inputs raise instead of fail-closed.)
...
if not result.verified:
    # Fail-closed gate: reject this input / route to human review.
    # ValueError (catchable per-request), not SystemExit (kills the host).
    # Never reach computed_cap / difference here (they are None).
...
# On unreleased main, result.status == "COMPUTED_ONLY" here
# (period computed, no claim to verify against). Published 0.4.0
# has no status field — gate on verified instead.
```

### Result status taxonomy

> `StatuteResult.status` and the fail-closed behaviors below track `qwed-legal` `main` (unreleased, post-`0.4.0`). Against the published `0.4.0` package, `status` is absent — gate on `verified` instead.
