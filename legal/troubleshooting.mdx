...
### `UNVERIFIABLE` for vague legal terms

**Problem:** A term like `"within a reasonable period"`, `"promptly after notice"`, or `"forthwith"` returns `verified=False` with `is_computable=False` and a message starting with `⚠️ UNVERIFIABLE`.

**Cause:** This is the intended fail-closed behavior. `DeadlineGuard` will not fabricate a deadline from ambiguous language. A term must include both a numeric quantity and a recognized time unit (`day`, `week`, `month`, `year`, optionally with a `business`/`working` qualifier).

**Solution:** Either rewrite the term with an explicit quantity and unit, or treat the result as a signal that the clause requires human legal interpretation:

```python
result = guard.verify("2026-01-01", "promptly", "2026-01-15")

if not result.is_computable:
    # Route to human reviewer — do not auto-approve
    flag_for_legal_review(result.message)
else:
    use_computed(result.computed_deadline)
```

Inputs like `"30"` (no unit) or `"15 intervals"` (unrecognized unit) also fail closed for the same reason.
