...
A valid format result does **not** prove that the cited authority exists or is controlling. `result.verified` is always `False` and `result.status` is `unverifiable_authority` whenever the format matches — CitationGuard has no case-law database. It only confirms the citation matched a supported structural pattern.
