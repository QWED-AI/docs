...
<Note>
  **Decimal input and output contract.** Cross-border guards (`DTAAGuard`, `TransferPricingGuard`, `PoEMGuard`) accept numeric inputs as `int`, `float`, `str`, or `Decimal`. Values are normalized to finite `Decimal` before any math. Booleans, `NaN`, and infinite values are rejected. Monetary and ratio fields in responses are returned as plain-text Decimal strings (for example, `"150.0"`, not `150.0`) to preserve exact precision across JSON boundaries.
</Note>
...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `foreign_income` | `int`, `float`, `str`, or `Decimal` | Yes | — | Income earned in foreign jurisdiction. Must be non-negative and finite |
| `foreign_tax_paid` | `int`, `float`, `str`, or `Decimal` | Yes | — | Tax paid in foreign jurisdiction. Must be non-negative and finite |
| `home_tax_rate` | `int`, `float`, `str`, or `Decimal` | Yes | — | Home country tax rate (percentage). Must be non-negative and finite |
| `foreign_tax_limit_rate` | `int`, `float`, `str`, or `Decimal` | No | `None` | DTAA treaty rate limit (percentage). When `None`, no treaty cap is applied. Must be non-negative when provided |

<Note>
  Monetary fields (`allowable_credit`, `excess_tax_lapsed`) are returned as plain-text Decimal strings (for example, `"150.0"`) to preserve exact precision. Convert them with `Decimal(...)` before arithmetic — do not cast to `float`.
</Note>

<Warning>
  Non-numeric, boolean, `NaN`, infinite, or negative inputs fail closed. The guard returns `{"verified": false, "message": "<field> must be a ... numeric value.", "allowable_credit": "0", "excess_tax_lapsed": "0"}` instead of computing a credit.
</Warning>
...
#  "message": "Price 80 deviates from ALP 100 beyond 3% tolerance.",
#  "safe_harbour_range": ["97.00", "103.00"], "potential_adjustment": "20.00"}
```

| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `transaction_price` | `int`, `float`, `str`, or `Decimal` | Yes | — | Actual price charged to/by related party. Must be finite |
| `benchmark_price` | `int`, `float`, `str`, or `Decimal` | Yes | — | Arm's Length Price (ALP) from analysis. Must be finite |
| `method` | `str` | No | `"CUP"` | Transfer pricing method (e.g., CUP, TNMM) |
| `tolerance_percent` | `int`, `float`, `str`, or `Decimal` | No | `Decimal("3.0")` | Safe harbour tolerance percentage. Must be finite |

<Note>
  `safe_harbour_range` and `potential_adjustment` are returned as plain-text Decimal strings so that downstream consumers can reconstruct exact values without float rounding.
</Note>

<Warning>
  Non-numeric, boolean, `NaN`, or infinite inputs fail closed. The guard returns `{"verified": false, "risk": "INVALID_NUMERIC_INPUT", "message": "<field> must be a ... numeric value.", "safe_harbour_range": [], "potential_adjustment": "0"}`.
</Warning>
...
#  "metrics": {"assets_outside_ratio": "0.2", "employees_outside_ratio": "0.2",
#              "payroll_outside_ratio": "0.2"},
...
| `turnover_total` | `int`, `float`, `str`, or `Decimal` | Yes | Total turnover. Must be finite |
| `turnover_outside_india` | `int`, `float`, `str`, or `Decimal` | Yes | Turnover outside India. Must be finite |
| `assets_total` | `int`, `float`, `str`, or `Decimal` | Yes | Total assets. Must be non-negative and finite |
| `assets_outside_india` | `int`, `float`, `str`, or `Decimal` | Yes | Assets outside India. Must be non-negative, finite, and not exceed `assets_total` |
| `employees_total` | `int` | Yes | Total employee count. Must be non-negative |
| `employees_outside_india` | `int` | Yes | Employees outside India. Must be non-negative and not exceed `employees_total` |
| `payroll_total` | `int`, `float`, `str`, or `Decimal` | Yes | Total payroll expense. Must be non-negative and finite |
| `payroll_outside_india` | `int`, `float`, `str`, or `Decimal` | Yes | Payroll expense outside India. Must be non-negative, finite, and not exceed `payroll_total` |
| `key_management_location` | `str` | Yes | Where key management decisions are made |

<Note>
  `metrics` values (`assets_outside_ratio`, `employees_outside_ratio`, `payroll_outside_ratio`) are quantized to four decimal places and returned as plain-text Decimal strings. The ABOI test itself uses the unrounded ratios internally, so decisions at the 0.50 threshold are not affected by display rounding.
</Note>

<Warning>
  PoEM inputs now fail closed. Any of the following return `{"verified": false, "residency": "UNVERIFIABLE", "reason": "..."}`:

  - Non-numeric, boolean, `NaN`, or infinite turnover, asset, or payroll values.
  - Negative asset, payroll, or employee values.
  - `assets_outside_india > assets_total`, `payroll_outside_india > payroll_total`, or `employees_outside_india > employees_total`.
</Warning>
