...
| Legal | Any | Body Corporate / Partnership | RCM (Recipient) |
| Security | Non-Body Corporate | Body Corporate | RCM (Recipient) |
| Director | Any | Body Corporate | RCM (Recipient) |
| Sponsorship | Any | Body Corporate / Partnership | RCM (Recipient) |
| Renting of motor vehicle | Non-Body Corporate | Body Corporate | RCM (Recipient) |
| Import of service | Any | Any (recipient in India) | RCM (Recipient) |
...
#  "audit_trace": {"rule_id": "RCM_LEGAL", "statute": "Notification 13/2017-CT(R), Sl. 2 (Legal)", ...}}
```

<Note>
  `verify_rcm_applicability` accepts either enum members or their raw string values (e.g. `"LEGAL"`, `"BODY_CORPORATE"`), so JSON-sourced payloads work without pre-conversion. Unknown values resolve to forward charge.
</Note>
...
- **Intra-state** (supplier state == place of supply): `CGST = SGST = value × rate / 2`, and `IGST` must be `0`.
- **Inter-state** (supplier state != place of supply): `IGST = value × rate`, and `CGST`/`SGST` must be `0`.

A small rounding tolerance (2 paise) applies only to tax-carrying CGST/SGST legs. Legs that must be exactly zero (the wrong tax type for the supply) get **no** tolerance, so even a tiny wrong-type amount is rejected. Negative claimed amounts fail closed.
