...
description: "Reference for QWED Tax guards covering US and India workflows including classification, nexus, payroll, withholding, GST, transfer pricing, and PoEM verification."
---
...
#  "safe_harbour_range": [97.0, 103.0], "potential_adjustment": 20.0}
```

| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `transaction_price` | `float` | Yes | — | Actual price charged to/by related party |
| `benchmark_price` | `float` | Yes | — | Arm's Length Price (ALP) from analysis |
| `method` | `str` | No | `"CUP"` | Transfer pricing method (e.g., CUP, TNMM) |
| `tolerance_percent` | `float` | No | `3.0` | Safe harbour tolerance percentage |
...
| `turnover_outside_india` | `float` | Yes | Turnover outside India |
| `assets_total` | `float` | Yes | Total assets |
| `assets_outside_india` | `float` | Yes | Assets outside India |
| `employees_total` | `int` | Yes | Total employee count |
| `employees_outside_india` | `int` | Yes | Employees outside India |
| `payroll_total` | `float` | Yes | Total payroll expense |
| `payroll_outside_india` | `float` | Yes | Payroll expense outside India |
