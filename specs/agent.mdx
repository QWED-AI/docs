...
<Warning>**v1.1.0:** The `context` field with `conversation_id` and `step_number` is now **required**. Requests without a valid context are rejected with `QWED-AGENT-CTX-001`.</Warning>
...
| `QWED-AGENT-CTX-001`    | Missing action context (`conversation_id` and `step_number` required) |
| `QWED-AGENT-CTX-002`    | Invalid step number (must be >= 1) |
| `QWED-AGENT-LOOP-001`   | Conversation step limit exceeded (max 50) |
| `QWED-AGENT-LOOP-002`   | Replay or out-of-order step detected |
| `QWED-AGENT-LOOP-003`   | Repetitive action loop detected |
