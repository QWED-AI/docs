...
1. Stop the process currently listening on the server port. It's either a stale QWED server started with different secrets or an unrelated process.
2. Re-run `qwed init`. It starts a fresh server with the secrets from your `.env`.

If the message says local secrets are missing, re-run `qwed init` so it generates and persists fresh secrets. See [`qwed init`](/advanced/cli#qwed-init-onboarding-and-bootstrap).
