...
This generic message is returned when the Stats Engine fails to generate or translate your query into executable code. The actual error details are masked to prevent leaking sensitive information.
...
This error occurs during Step 3 of `qwed init`. The local server tries to create a bootstrap SQLite database in a directory that is not writable — for example, a read-only mount, a container filesystem, or a system-managed path.
