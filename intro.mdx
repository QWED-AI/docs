...
<Note>
**QWED v7.2.0** is now live — Security hardening batch + precision advisory: fail-closed RCE/auth/sandbox fixes and a new float-precision advisory, with no breaking wire changes. [See what's new →](/releases)
</Note>
...
<AccordionGroup>
  <Accordion title="Security Hardening Batch" icon="shield-check" defaultOpen>
    Fail-closed fixes across expression parsing, auth, sandbox containment, and event-loop safety — including RCE fixes, auth DoS hardening, engine-call bounds, and dependency modernization. No breaking wire changes. [Learn more →](/releases)
  </Accordion>
  <Accordion title="Precision Advisory" icon="microchip">
    New additive capability: binary floating-point constants in math/stats verification surface a `precision.float-constants` advisory in `developer_fields.advisory_checks` without affecting verdicts.
  </Accordion>
</AccordionGroup>
