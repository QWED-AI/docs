...
| Claude writes SQL → unverified for injection | Script uses `qwed_new` SQL analyzer → **injection detected** |
| Claude reasons → unverified | Z3 solver executed via SDK → **formally proven** |
| Claude codes → unverified for safety | AST check script executed → **security checked** |
