...
<Warning>
  If you see a `BLOCKED` response with error code `QWED-MCP-RISK-001` in Claude Desktop, it means the LLM is trying to call a tool not in the governance policy table (typically a removed v0.1.x tool). Tell Claude: "The verify\_\* tools have been removed. Use execute\_python\_code to write and run a Python verification script."
</Warning>
...
| `verification_id` | `str` | Context-bound SHA-256 fingerprint of the tool name, normalized arguments, a per-call nonce, wall-clock timestamp, and the server instance ID. Unique to each call, even for identical requests. |
...
<Info>
  The `verification_id` is a context-bound SHA-256 hash that mixes the tool name and normalized arguments with a random per-call nonce, a wall-clock timestamp, and a per-process server instance ID. Two identical requests always produce **different** `verification_id` values, which prevents replay attacks and stale-cache correlation. Use it as a per-call audit identifier rather than a deduplication key.
</Info>
